...
Last updated: 2026-06-03 · branch: `main` · latest tag: **`p6b-session1a-drift-complete`** (`fe2d3ab`) — **P6b §1a-drift merged via PR #56** (squash `fe2d3ab`): **strategy drift detection + shared metrics**, the first P6b session (capabilities deferred from P6 Direction v0.2). Advisory-only: compares a live strategy's recent behavior to its backtest baseline and surfaces divergence on the morning-brief cadence (Mon-Fri 09:00 ET) — takes no action. New `app/strategies/metrics.py` extracts the formula FUNCTIONS (`win_rate`/`avg_return_per_trade`/`sharpe_ratio`/`max_drawdown`) so the backtester + drift detector share bit-identical math; **`BacktestMetrics` stays in `backtest_models.py`** (metrics_json contract untouched) — "extract functions, keep the dataclass". `app/services/drift_detection.py`: **fill-level FIFO round-trip reconstruction** (`Fill→Order→Symbol`; Order has NO fill aggregates — fills are the legs), **params-matched baseline from `BacktestResult.trades_json`**, **sizing-invariant compare** on win_rate + avg_return_per_trade (dollar avg-pnl breaches on position sizing alone). Live Sharpe/max-dd deferred (degenerate at small N + need an equity-curve reconstruction). New **`STRATEGY_DRIFT_DETECTED`** audit (6th P6-era action); written **one row per finding, one-row-per-txn** (hash-chain contract). Thresholds in `agent_envelope_json.drift_thresholds` (win_rate ±10pp, avg_return ±25% rel, min_trades 20, lookback 30d). **3 EXECUTION DEVIATIONS vs the v0.1 doc:** (1) **audit-only, NO migration** — `morning_briefs` has no `report_json` column (the doc's persistence target didn't exist); brief rendering = §1b; (2) one-row-per-txn commits (multiple-per-commit unchains the audit hash); (3) `_canonical_params` rewritten — `json.dumps(default=str)` does NOT normalize int-vs-str (recursive leaf-stringify instead). No new MCP tool/frontend/order-path code (all §1b-drift). 32 backend tests; full suite 0 failed; mypy(158) + ruff + all 8 shell invariants + audit-immutability + 3 coverage gates green; all 10 PR CI green first try; merged on Jay's "merge it" (informed ≥1h walk-away override). **Deferred** (Norton + no Docker): live Mon-Fri 09:00 ET brief with real fills/backtest → drift firing. Session doc + Results: `TradingWorkbench_P6b_Session1a_drift_v0_1.md` / `..._Session1a_drift_Results_v0.1.md`. · **Next: P6b §1b-drift** — on-demand `POST /strategies/{id}/drift-check` endpoint + `workbench_drift_findings` MCP tool (17→18) + frontend (morning-brief drift section + strategy-detail drift card); draft `..._P6b_Session1b_drift_v0.1.md` against the §1a Results (NOT speculatively). Then P6b §2 (auto-validation paper-variant runner, ADR 0007). Still pending (live, non-Norton stack): **§1b.12 → `p6-session1-complete`** + §2b live re-confirmation. · prior tags `p6-session2b-complete`, `p6-session2b-review-complete` (`2ee3c93`), `p6-session2b-backtest-complete` (`267b666`), `p5.5-complete` (`878b9f8`)
