...
Last updated: 2026-06-03 · branch: `main` · latest tag: **`p6-session2b-review-complete`** (`2ee3c93`) — **P6 §2b-review merged via PR #55** (squash `2ee3c93`): **10% human-review sampling + review UI**, closing Decision 8's eval-harness story (backtest = primary ground truth, human review = qualitative supplement). New `app/services/proposal_review_sampling.py`: singleton weekly cron (Mon 09:00 ET, registered lifespan 10h) samples 10% (floor; all if <10; none if 0) of each user's past-7-days terminal-state proposals, with **two non-negotiable invariants** — de-dup vs already-sampled, and merge-not-overwrite of `evaluation_results_json` (preserves the §2b-backtest eval sub-tree) — each with a dedicated test. `POST /proposals/{id}/review` validates sampled+not-yet-rated, merge-writes `human_review.reviewed_at/rating/reason`, writes new `PROPOSAL_REVIEW_RECORDED` audit (5th P6-era action; sampling is silent, the user's review is the audited event). `GET /proposals?awaiting_review=true` (`func.json_extract` Core filter); additive `n_reviewed/n_thumbs_up/n_thumbs_down` on eval-summary (MCP `workbench_proposal_eval_summary` passthrough, tool count stays 17). Frontend `ReviewQueue` page (`/proposals/review` explicit `<Route>`, not nav) reusing `EvalPanel` + awaiting-review badges on Proposals header & Dashboard card. No new migration/MCP tools/ADRs; no order-path code. **KEY DRIFT vs v0.1 doc** (caught by grepping live code first): lifespan `app.state.scheduler` was UNSET at the registration point → used in-scope `scheduler.scheduler`+`session_factory` (mirrors §2b-backtest reconcile); list endpoint kept `ProposalListResponse`; frontend is `@tanstack/react-query` **v5** not v3; `EvalPanel` lives at `@/components/proposals/EvalPanel` with prop `ev`. 31 backend + 6 frontend tests; full backend suite (0 failed) + vitest 107 + mypy(156) + ruff + all invariants + 3 coverage gates green; all 10 PR CI checks green first try; merged on Jay's "merge now" (informed ≥1h walk-away override, CI green). **Deferred** (Norton + no Docker): live Mon-09:00-ET firing + real-Anthropic/Docker §2b cross-session loop. (P6 §2b closed: `p6-session2b-complete` tagged on the in-suite stand-in this session). · **Next: P6b** — the four capabilities deferred from P6 Direction v0.2 (drift detection; auto-validation paper-variant runner [ADR 0007]; evidence-bundle promotion-approval flow [ADR 0007]; Mode-B LLM-eval harness + LLM-driven live opt-in [ADR 0006 v2]) are **committed product requirements, NOT empirical polish** — scheduled as P6b (~27–28h/5 sessions; **committed phased split: P6b-early §1–§3, P6b-late §4–§5 [Mode-B + LLM-driven live] replanned with a fresh v0.1 after §3 ships**; **drift detection FIRST** (~5–6h, advisory; its detection-method — rolling Sharpe / KS-test / variance ratio + baseline source + cadence — is a §1-planning architectural decision; P7 plans an integration point with it, NOT a hard blocker)), sequenced BEFORE P7. Charter = Part D of `TradingWorkbench_P6_Closure_Observation_Plan_v0_1.md`; full design already in Direction v0.2 + ADR 0006 v2 + ADR 0007. The 4-week observation framework (Closure plan Part B) runs in parallel and *tunes* P6b thresholds, does not gate it. Still pending (live, non-Norton stack): **§1b.12 → `p6-session1-complete`** (only genuinely-pending tag) + §2b live re-confirmation. · prior tags `p6-session2b-backtest-complete` (`267b666`), `p6-cleanup-1-complete` (`6fb0f9f`), `p5.5-complete` (`878b9f8`)
