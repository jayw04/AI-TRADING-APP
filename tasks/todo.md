...
Last updated: 2026-06-03 · branch: `main` · latest tag: **`p6-session2b-backtest-complete`** (`267b666`) — **P6 §2b-backtest merged via PR #54** (squash `267b666`): **backtest eval pipeline**. On DRAFT→REVIEWING the PATCH endpoint enqueues two `BacktestJob` rows (baseline current params + variant merged with the proposal's `changes`, 90-day window) via a service-layer direct insert (sidesteps the HTTP single-flight 409); a 60s reconcile cron (`proposal_evaluation_reconcile`, inside the alpaca block) detects completion via `BacktestJob.result_id` and writes Decision 8's verdict (`sharpe ≥ baseline AND drawdown ≥ max(baseline−0.05, −0.20)`) into `evaluation_results_json`. Non-PYTHON/zero-symbol strategies skip with a reason; eval is judgment fuel, not a gate. `GET /strategies/{id}/proposal-eval-summary` + `workbench_proposal_eval_summary` MCP tool (16→17) + frontend eval badge/`EvalPanel`. No new migration/audit/ADRs (ADR-0014 already Accepted); no order-path code. All 10 PR CI checks green first try. **Deferred** (Norton + no Docker): real Anthropic proposal + live worker run. · **Next: P6 §2b-review** (10% human-review sampling cron + review UI; draft `..._P6_Session2b_review_v0.1.md` against the §2b-backtest Results). Still pending: **§1b.12 → `p6-session1-complete`** on a non-Norton stack. · prior tags `p6-cleanup-1-complete` (`6fb0f9f`), `p6-session2a-complete` (`d0f709e`), `p5.5-complete` (`878b9f8`)
