...
Last updated: 2026-06-02 · branch: `main` · latest tag: **`p6-session1b-complete`** (`16855f9`) — **P6 §1b merged via PR #51** (squash `16855f9`): the agent now generates proposals end-to-end. `apps/agent/` FastAPI control-plane (`POST :8767/generate-proposal`) + real MCP-over-SSE read client + backend write client + proposal-generation flow; backend `POST /strategies/{id}/propose`, `PATCH /proposals/{id}`, `GET /proposals`, `POST /proposals/{id}/apply` (merges `params_json`, IDLE guard), `GET /strategies/{id}/history` (in proposals.py); 4 new workbench-mcp GET tools (12→16); frontend Proposals page + RecentProposalsCard + envelope editor (6th profile section) + `/proposals` route; **ADRs 0009-0014**. No new migration/audit actions; no order-path code touched. All 10 PR CI checks green (after fixing a P2-coverage-gate dip + a frontend dup-link test — see §1b deviations). **Deferred** (Norton + no Docker): real Anthropic call, `docker compose` run, **§1b.12 cross-session verification → `p6-session1-complete` tag** · **next: run §1b.12 on a non-Norton stack, then P6 §2** (scheduled cadence + backtest eval) · prior tags `p6-session1a-complete` (`b95395a`), `p5.5-complete` (`878b9f8`)
