...
### The eighteen CI invariants are load-bearing

Listed in roughly order of merge: ADR 0002 single-router, strategy isolation, risk coverage ≥95%, P2 module coverage, P3 module coverage, MCP read-only, broker isolation, no env credentials, audit immutability, workbench-MCP read-only, no-LLM-in-order-path, eval-harness-paper-only (P6b §4 — the LLM eval harness never routes orders to a live account, ADR 0006 v2), llm-opt-in-bypass-gated (P6b §5 — the only sanctioned LLM-in-order-path fires only behind an `active` `llm_opt_in` DB row + version pin + per-user cap, ADR 0006 v2 §5), altdata-order-path-isolation (ADR 0037 — the EAD alt-data / Security-Master / opportunity-report packages import no order-path module; `check_altdata_order_path_isolation.sh`, lands with Phase 1 EAD ingestion), and ead-reference-only (2026-07-07 — the converse of the previous: the order-path / ranking / selection modules name no *rejected-EAD* event label (`insider_buy`/`gov_contract_award`/`congress_trade`/`lobby_spike`); a rejected EAD pattern may be displayed as context but must never enter ranking, sizing, or the order path; `check_reference_only_invariant.sh` + the `app/altdata/reference_only.py` guard; EAD Dataset Triage v0.2), and loss-control-structural (ADR 0043 PR5 — the persisted loss-control architecture: single-persister (only `app/risk/loss_control/service.py` writes the state-machine tables), engine-only gate, classifier reuse (no ADR-0042-classifier import in `loss_control/`), no implicit state bootstrap at the decision seam, and no premature recovery/re-arm triggers in the engine; AST-based `check_loss_control_invariants.sh`), and settlement-barrier (ADR 0043 — an ADR-0043 governed harness cannot express "submit an order without settling it": every `scripts/adr0043_*.py` places orders through the single `GovernedSubmitter` seam, which pairs the submit with the shared per-order REST barrier, so no harness calls `router.submit` or `settle_order` directly and the seam itself provably reaches the barrier; AST-based `check_settlement_barrier.sh`. Governs the ADR-0043 harnesses only — the LIVE order path still relies on the trade-updates stream plus `reconcile_stuck_orders`, and making every production submit block on a synchronous REST poll would be an order-path behaviour change requiring its own ADR), and aws-sdk-isolation (ADR 0046 — boto3 is a direct dependency so the forward-validation witness can sign in AWS KMS, and the property that makes that safe is not the SDK's absence but its *unreachability*: an ALLOWLIST, like the no-LLM invariant, permits `boto3`/`botocore`/`aiobotocore`/`types_boto3*`/`mypy_boto3*` imports under `app/validation/aws/` alone — type-only imports included — and separately forbids anything outside that package from importing `app.validation.aws`, so the adapters are reachable only through the `witness.signer.factory` / `witness.sink.factory` strings in the governed deployment configuration; `check_aws_sdk_isolation.sh`). Each enforces a property that cannot be re-introduced without breaking trust the platform has spent years earning. Disabling an invariant requires an ADR.
