...
`mcp.server.fastmcp`, which `mcp_workbench.server` imports. Because the dependency was
...
# pyproject.toml. Keep in sync with apps/mcp-server.
PINNED_MCP_VERSION = "1.28.1"


def test_fastmcp_import_surface_is_available() -> None:
    """The precise import `mcp_workbench.server` depends on must resolve."""
...
def test_server_module_imports_with_the_pinned_dependency() -> None:
    ...
    pytest.importorskip("mcp_workbench.server")
    import mcp_workbench.server as srv
