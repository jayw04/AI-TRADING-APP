...
exactly one row must match the form, or the parse fails closed.
"""
...
def parse_primary_document(raw: bytes, form: str) -> PrimaryDocument:
    ...
    Exactly one row must carry ``Type == form``. There is no fallback: a page that does not
    satisfy the contract fails closed rather than being reinterpreted.
    """
    ...
    matched: list[PrimaryDocument] = []
    for row in rows:
        cells = _TD.findall(row)
        if len(cells) <= max(idx.values()):
            continue
        if _text(cells[idx["Type"]]) != form:
            continue
    ...
    if not matched:
        raise FilingDetailError(
            NO_PRIMARY_ROW, f"no row in the document table has Type == {form!r}"
        )
    if len(matched) > 1:
        raise FilingDetailError(
            AMBIGUOUS_PRIMARY_ROW,
            f"{len(matched)} rows have Type == {form!r}: {[m.document for m in matched]}",
        )
    return matched[0]
