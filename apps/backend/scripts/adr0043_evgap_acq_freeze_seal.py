#!/usr/bin/env python3
"""Seal ADR0043-PH0-D-BOX-EVIDENCE-GAP-ACQ-FREEZE-001 (acquisition freeze).

Governance/tooling only. Does not inspect candidate observation rows, capture
mutable snapshots, reconstruct surfaces, or authorize acquisition-start.
Uses RFC8785-JCS + SHA-256(manifest_body) identical to campaign/O34 freeze tooling.
"""
...
DOC_ID = "ADR0043-PH0-D-BOX-EVIDENCE-GAP-ACQ-FREEZE-001"
UNSEALED_PATH = (
    ROOT / "docs/design/ADR0043_Phase0_D_BOX_Evidence_Gap_ACQ_Freeze_Manifest_001_UNSEALED.json"
)
SEALED_PATH = (
    ROOT / "docs/design/ADR0043_Phase0_D_BOX_Evidence_Gap_ACQ_Freeze_Manifest_001_SEALED.json"
)
...
def build_manifest_body(*, content_tip_commit: str) -> dict:
    schemas = {
        "O3": {
    ...
    return {
        "document_purpose": (
            "EVIDENCE-GAP acquisition freeze — precommits eligibility, source "
            "classes, contract-level field mappings, reconstruction/prohibited-"
            "inference rules, completeness/exclusion codes, planned count "
            "equations, schemas, O4-A/O4-B no-mix, and O5 locate-only protocol "
            "before any record selection, reconstruction, or capture"
        ),
        "governing_refs": {
            ...
            "qual001_archive_ids_forbidden_reuse": [
                ...
                "O4B-CAND-20260730T022316Z",
            ],
            "prior_o34_acq_freeze_id": "ADR0043-PH0-D-BOX-O34-ACQ-FREEZE-001",
            "prior_o34_acq_freeze_body_sha256": (
            ...
            "content_tip_commit": content_tip_commit,
            "seal_tooling_path": seal_tool_path,
            "seal_tooling_path_sha256": file_sha(seal_tool_path),
        },
        "account_3_identity": {
        ...
        "hold_and_blocks": {
            ...
            "prospective_instrumentation_new_evidence_population": "NOT_AUTHORIZED",
            ...
            "evidence_row_access_before_seal_and_start": "FORBIDDEN",
        },
        "sequence_gate": {
            "required_order": [
                "FILL",
                "PUBLISH_CONTENT_TIP",
                "REBIND_COMMIT_TOOL_SCHEMA_IDENTITIES",
                "READINESS_VALIDATION",
                "SEAL",
                "COUNTERSIGN",
                "SEPARATE_ACQUISITION_START_DECISION",
                "SNAPSHOT_CAPTURE",
                "RECORD_SELECTION",
                "CONSTRUCTION",
            ...
            "snapshot_capture_authorized_by_this_document": False,
            "exploratory_selection_before_seal": "FORBIDDEN",
            "source_inventory_inspection_for_selection_before_seal": "FORBIDDEN",
            "candidate_row_queries_before_start": "FORBIDDEN",
            "o5_anchor_content_search_before_start": "FORBIDDEN",
        },
        "time_standard": {
            "timezone": "UTC",
            "format": "RFC3339",
        ...
        "eligibility_window": {
            ...
            "end_exclusive_utc": "2026-07-30T19:39:07Z",
            "bounds_policy": "start_inclusive_end_exclusive",
            "start_basis": (
                "ADR0032_AWS_paper_cutover_era_bound_from_governing_ops_docs_"
                "exogenous_not_outcome_inspection"
            ),
            "end_basis": (
                "EVIDENCE-GAP-ACQ-AUTH-001 publication merge timestamp "
                "2026-07-30T19:39:07Z (PR #571 / 29eece3) — exclude "
                "post-authorization observations; exogenous to data quality"
            ),
        },
        "source_inventories": {
            "policy": (
                "Pin read-only locations and snapshot-binding protocol without "
                "selecting observation rows or inspecting candidate content. "
                "Live mutable stores require acquisition-start capture of exact "
                "file/object SHA-256 before selection. Capture is not selection. "
                "Locations reuse prior sealed O34 inventory identities as "
            ...
            "entries": [
                {
                    ...
                    "read_only_location": (
                        "Lawful local stores under prior Workbench authority: "
                        "/opt/workbench/data/bar_cache and "
                        "/opt/workbench/data/market_projection (concrete object "
                        "Version IDs / tree SHA-256 bound at acquisition-start "
                ...
                },
                {
                    ...
                    "read_only_location": (
                        ...
                        "(tables/classes previously published: orders, fills, "
                        "accounts_state, audit_log, checkpoint/terminal package "
                        "artifacts). Production stack b0058bf reference-only; "
                        "must not be modified. SHA-256 of sqlite bytes or "
                ...
                {
                    "source_id": "SRC-O5-TIERA-LOCATE-CORPUS-001",
                    "source_class": "preexisting_sealed_tier_a_live_fill_anchors",
                    "read_only_location": (
                        "Governing git design/evidence trees and any "
                        "previously sealed Tier-A live-fill anchor manifests "
                        "already published under ADR-0043 Phase-0 authority; "
        ...
        "source_field_to_target_surface_mappings": {
            "mapping_basis": (
                "Defined from published archive schemas, phase0 Python "
                "contracts, and prior sealed source-class locations — not from "
                "candidate-row inspection"
            ),
            "O3": [
                {
                    "target_surface": "quote_provenance",
                    "target_contract": (
                        "app.risk.loss_control.phase0_quote_provenance."
                        "QuoteProvenance / ProvenancedQuote"
                    ),
                    "required_keys": [
                        "provider",
                        "feed_type",
                    ...
                    "source_field_classes": [
                        "quote payload bytes for plan symbol",
                        "structured provenance metadata satisfying REQUIRED_FIELDS",
                    ],
                    "as_of_constraint": (
                        "as-of <= reconstructable replay/decision time for the "
                        "plan from bound snapshot"
                    ),
                    "reconstruction_rule": (
                        "Deterministic projection into QuoteProvenance.as_dict "
                        "and ProvenancedQuote; refuse string-only sources"
                    ),
                    "prohibited_inference": [
                        "synthesize quotes",
                        "later-market backfill",
                        "evaluator defaults",
                        "string-only provenance",
                    ],
                    "incomplete_reason_code": "MISSING_REPLAY_SURFACE:quote_provenance",
                },
                {
                    "target_surface": "checkpoint_tuple",
                    "target_contract": (
                        "app.risk.loss_control.phase0_checkpoint.CheckpointBinding"
                    ),
                    "required_keys": [
                        "schema_version",
                        "run_id",
                    ...
                    "source_ids": ["SRC-APP-AUDIT-PLAN-CKPT-TERM-001"],
                    "source_field_classes": [
                        "sealed checkpoint envelopes / binding tuples for account 3"
                    ],
                    "as_of_constraint": (
                        "checkpoint must bind to the same plan/episode; "
                        "content_hash required on sealed envelope"
                    ),
                    "reconstruction_rule": (
                        "Deterministic load via checkpoint integrity rules; "
                        "fail closed on incomplete binding or hash mismatch"
                    ),
                    "prohibited_inference": [
                        "evaluator default checkpoints",
                        "cross-session reuse",
                        "fabricated loss_control_state_version",
                    ],
                    "incomplete_reason_code": "MISSING_REPLAY_SURFACE:checkpoint_tuple",
                },
                {
                    "target_surface": "loss_accounting_inputs",
                    "target_contract": (
                        "app.risk.loss_control.phase0_loss_accounting.FillLeg + "
                        "LossAccountingPolicy (ADR0043-PH0-LOSS-001)"
                    ),
                    "required_keys": [
                        "legs_or_equivalent_inputs",
                        "formula_id",
                    ],
                    "source_ids": [
                        "SRC-APP-AUDIT-PLAN-CKPT-TERM-001",
                    ...
                    "source_field_classes": [
                        "fill/leg economics attributable to plan",
                        "policy slots for ADR0043-PH0-LOSS-001",
                    ],
                    "as_of_constraint": (
                        "inputs must pre-exist in bound snapshot; no post-hoc "
                        "invention of fees/prices"
                    ),
                    "reconstruction_rule": (
                        "Deterministic assembly of FillLeg sequence + policy "
                        "dict sufficient for compute_canonical_loss"
                    ),
                    "prohibited_inference": [
                        "invented legs",
                        "assumed fees",
                ...
                },
                {
                    ...
                    "target_contract": (
                        "app.risk.loss_control.phase0_crash_consistency."
                        "TerminalPackage (governed recovery object for O3)"
                    ),
                    "required_keys": [
                        "run_id",
                        "account_id",
                    ...
                    "prohibited_inference": [
                        "fabricated recovery status",
                        "assumed conclusive=true",
                        "invented digests",
                    ],
                    "incomplete_reason_code": "MISSING_REPLAY_SURFACE:recovery_inputs",
                },
                {
                    "target_surface": "authority_inputs",
                    "target_contract": "order/plan authority dict retained from O34 family",
                    "required_keys": ["account_3_scoped_authority_object"],
                    "source_ids": [
                        "SRC-APP-AUDIT-PLAN-CKPT-TERM-001",
                        "SRC-ACCT3-PAPER-PRIOR-AUTH-001",
                    ],
                    ...
                    "prohibited_inference": ["cross-account authority"],
                    "incomplete_reason_code": "MISSING_REPLAY_SURFACE:authority_inputs",
                },
                {
                    "target_surface": "plan_id",
                    "target_contract": "phase0_o34_archive_adapter.parse_ord_plan_id",
                    "required_keys": ["plan_id"],
                    "source_ids": ["SRC-APP-AUDIT-PLAN-CKPT-TERM-001"],
                    "source_field_classes": ["orders.id mapped as ord:<orders.id>"],
                    "as_of_constraint": "n/a",
                    "reconstruction_rule": (
                        "plan_id=ord:<orders.id> unless superseding contract "
                        "amended into a future freeze"
                    ),
                    "prohibited_inference": ["non-ord synthetic identities"],
                    "incomplete_reason_code": "MISSING_PROVENANCE",
                },
            ],
            "O4_A": [
                {
                    "target_surface": "decision_time_quotes",
                    "target_contract": (
                        "phase0_o4_replay.DecisionTimeEvidence.quotes / "
                        "phase0_reachability two-sided bid/ask/age_s"
                    ),
                    "required_keys": ["bid", "ask", "age_s_or_equivalent_freshness"],
                    "source_ids": ["SRC-MKT-QUOTE-LAWFUL-001"],
                    "source_field_classes": [
                    ...
                    "reconstruction_rule": (
                        ...
                        "at/before cutoff from bound market stores; empty book → "
                        "exclude MISSING_DECISION_TIME_QUOTE"
                    ),
                    "prohibited_inference": [
                        "post_submit_quotes",
                        "O4-B fills/terminal import",
                        "later market data",
                        "evaluator default quotes",
                    ],
                    "incomplete_reason_code": "MISSING_DECISION_TIME_QUOTE",
                },
                {
                    "target_surface": "model_available",
                    "target_contract": "DecisionTimeEvidence.model_available:bool",
                    "required_keys": ["model_available"],
                    "source_ids": ["SRC-APP-AUDIT-PLAN-CKPT-TERM-001"],
                    "source_field_classes": [
                        "decision-time model-availability flag recorded for the plan"
                    ],
                    "as_of_constraint": "at decision time / at or before cutoff",
                    "reconstruction_rule": (
                        "Emit explicit boolean from pre-existing record; absence → "
                        "exclude MISSING_PROVENANCE (do not default true)"
                    ),
                    "prohibited_inference": ["default-true fabrication"],
                    "incomplete_reason_code": "MISSING_PROVENANCE",
                },
                {
                    "target_surface": "cutoff_at_utc",
                    "target_contract": "FIRST_BROKER_SUBMISSION_BOUNDARY",
                    "required_keys": ["cutoff_at_utc"],
                    "source_ids": ["SRC-APP-AUDIT-PLAN-CKPT-TERM-001"],
                    "source_field_classes": [
                        "orders.submitted_at or first broker submission attempt "
                        "attributable to plan; else last pre-submit decision event"
                    ],
                    "as_of_constraint": "reconstructable from bound snapshot only",
                    "reconstruction_rule": (
                        "Same family as O34-ACQ-FREEZE-001 o4a_cutoff_rule; "
                        "unreconstructable → MISSING_CUTOFF"
                    ),
                    "prohibited_inference": ["assumed cutoff", "post-hoc ordering"],
                    "incomplete_reason_code": "MISSING_CUTOFF",
                },
                {
                    "target_surface": "o4a_prohibited_fields",
                    "target_contract": "EVGAP O4-A schema fills maxItems=0; terminal/post_submit null",
                    "required_keys": [],
                    "source_ids": [],
                    "source_field_classes": [],
                    "as_of_constraint": "n/a",
                    "reconstruction_rule": (
                        "fills must be empty; terminal_broker_state and "
                        "post_submit_quotes must be null; day_change on O4-A rows "
                        "must remain null (forensic baseline belongs to O4-B)"
                    ),
                    "prohibited_inference": ["any O4-B payload merge into O4-A"],
                    "incomplete_reason_code": "CROSS_PLANE_MIX",
                },
            ],
            "O4_B": [
                {
                    "target_surface": "fills_and_terminal_completeness",
                    "target_contract": "O4B-TERM-COMPLETE-001 / ForensicEvidence.fills",
                    "required_keys": [
                        "fills_min_1",
                    ...
                    "source_field_classes": [
                        "fills table rows for plan/orders",
                ...
                {
                    "target_surface": "day_change",
                    "target_contract": (
                        "phase0_reachability.remaining_to_target / "
                        "ForensicEvidence.day_change under "
                        "LossAccountingPolicy.control_baseline_mode="
                        "BROKER_EQUITY (default Phase-0)"
                    ),
                    "required_keys": ["day_change", "baseline_mode", "as_of_utc"],
                    "source_ids": [
                        "SRC-ACCT3-PAPER-PRIOR-AUTH-001",
                        "SRC-APP-AUDIT-PLAN-CKPT-TERM-001",
                    ],
                    "source_field_classes": [
                        "account-3 broker equity / day P&L fields "
                        "(accounts_state.raw_payload or governed equivalent) "
                        "at forensic as-of"
                    ],
                    "as_of_constraint": (
                        "forensic as-of for the episode; provenance must record "
                        "baseline_mode and as_of_utc"
                    ),
                    "reconstruction_rule": (
                        "Deterministic extraction of decimal day_change under "
                        "bound baseline mode; null/absent → "
                        "MISSING_FORENSIC_BASELINE (not QUALIFIED for O4-B gate "
                        "binding unless freeze-accepted equivalent present)"
                    ),
                    "prohibited_inference": [
                        "assumption-filled day_change",
                        "O4-A-only substitution",
                        "later equity marks after forensic as-of",
                        "evaluator defaults",
                    ],
                    "incomplete_reason_code": "MISSING_FORENSIC_BASELINE",
                },
            ],
            "O5": [
                {
                    ...
                    "source_field_classes": [
                        "pre-existing sealed Tier-A live-fill anchor manifests"
                    ],
                    "as_of_constraint": "anchor must pre-exist; no new fills",
                    "reconstruction_rule": (
                        "Locate and qualify only after acquisition-start; "
                        "anchors:[] is a valid outcome"
                    ),
                    "prohibited_inference": [
                        "generate fills",
                        "shadow-session execution",
                        "broker submission",
                        "placeholder anchors",
                    ],
                    "incomplete_reason_code": "O5_ANCHORS_EMPTY_VALID",
                }
            ],
        },
        "inclusion_rules": [
            {
                ...
                "description": (
                    ...
                    "PA34USW0Q8UO) order/plan episodes with plan_created_at_utc "
                    "(or orders.created_at equivalent) in eligibility_window"
                ),
            },
            {
                "rule_id": "INC-002",
                "description": (
                    ...
                    "snapshot binding at acquisition-start"
                ),
            },
            {
                "rule_id": "INC-003",
            ...
            },
            {
                ...
                "description": (
                    ...
                    "O4B-TERM-COMPLETE-001 plus forensic baseline rule when "
                    "emitting QUALIFIED-bindable rows"
                ),
            },
            {
                "rule_id": "INC-005",
                "description": (
                    "O3 QUALIFIED-bindable rows require all replay surfaces "
                    "non-null: quote_provenance, authority_inputs, "
                    "checkpoint_tuple, loss_accounting_inputs, recovery_inputs"
                ),
            },
        ],
        "exclusion_rules": [
            ...
            {
                "rule_id": "EXC-002",
                "description": "Exclude test_adr0043_dbox_freeze_manifest.py stubs",
            },
            {
                "rule_id": "EXC-003",
                "description": (
            ...
            },
            {
                "rule_id": "EXC-008",
                "description": (
                    "Exclude Option 2A hermetic CORR-06/O1/O2 evidence root as "
                    "O3/O4 empirical observations"
                ),
            },
            {
                "rule_id": "EXC-009",
            ...
            },
            {
                "rule_id": "EXC-012",
                "description": (
                    "Exclude mutation/reuse of QUAL-001 archive IDs "
                    "O3/O4A/O4B-CAND-20260730T022316Z; those remain immutable "
                    "historical evidence"
                ),
            },
            {
                "rule_id": "EXC-013",
                "description": (
                    "Exclude CAMPAIGN v1.2 run evidence root "
                    "docs/design/evidence/dbox_campaign_v1_2_run_001/ as an "
                    "empirical acquisition source (reference dispositions only)"
                ),
            },
            {
                "rule_id": "EXC-014",
                "description": (
                    "Exclude fabricating gap surfaces from O34 constructed "
                    "archives that systematically null those surfaces"
                ),
            },
        ],
        "deduplication": {
            ...
            "plan_id_default": "ord:<orders.id>",
            "collision_policy": "FAIL_CLOSED_STOP_INCONCLUSIVE",
        },
        "unit_of_observation": {
            "definition": (
                "One order/plan episode (plan_id=ord:<orders.id>) for account 3 "
                "within eligibility_window"
            ),
        ...
        "symbol_session_clustering": {
            ...
            "independence_assumption": (
                ...
                "dependent; report n_raw and n_eff<=n_raw"
            ),
            "counting_policy": (
        ...
        "o4a_cutoff_rule": {
            ...
            "reconstruction_method": (
                ...
                "attributable to that plan from audit/order/terminal records in "
                "the bound source snapshot (orders.submitted_at family); if no "
                "submission occurred, cutoff is the last pre-submit decision "
                "event recorded for the plan; if neither can be reconstructed → "
                "exclude MISSING_CUTOFF (do not invent)"
            ),
            ...
            "no_mix_with_o4b": "MANDATORY_SEPARATE_ARCHIVES",
        },
        "o4b_terminal_completeness_rule": {
            ...
            "forensic_baseline_for_qualified_binding": (
                ...
                "baseline_mode + as_of_utc provenance) required for "
                "QUALIFIED O4-B gate binding"
            ),
            "incomplete_observation_treatment": (
                "EXCLUDE_WITH_REASON_CODE_O4B_INCOMPLETE or "
                "MISSING_FORENSIC_BASELINE; if exclusion would require "
        ...
        "o5_locate_only_protocol": {
            "authorized_actions": [
                "locate_preexisting_sealed_tier_a_anchors",
                "independently_qualify_located_anchors",
            ],
            "forbidden_actions": [
                "new_orders",
                "generate_new_fills",
                "shadow_session_execution",
                "broker_submission",
            ],
            "empty_anchors_policy": "anchors:[] VALID → predetermined INCONCLUSIVE",
            "content_search_before_acquisition_start": "FORBIDDEN",
        },
        "completeness_criteria": {
            "O3_QUALIFIED_BINDABLE": (
                "All of quote_provenance, authority_inputs, checkpoint_tuple, "
                "loss_accounting_inputs, recovery_inputs non-null and contract-"
                "valid"
            ),
            "O4A_QUALIFIED_BINDABLE": (
                "Reconstructable cutoff; non-empty two-sided decision-time quotes "
                "per intended symbol with freshness; explicit model_available; "
                "fills empty; no O4-B fields"
            ),
            "O4B_QUALIFIED_BINDABLE": (
                "O4B-TERM-COMPLETE-001 plus non-null day_change (or accepted "
                "equivalent) with baseline provenance"
            ),
            "O5": "Tier-A anchors located and qualified, or anchors:[] valid",
        },
        "missing_data_treatment": {
            ...
            "reason_codes": [
                ...
                "MISSING_REPLAY_SURFACE:quote_provenance",
                "MISSING_REPLAY_SURFACE:checkpoint_tuple",
                "MISSING_REPLAY_SURFACE:loss_accounting_inputs",
                "MISSING_REPLAY_SURFACE:recovery_inputs",
                "MISSING_REPLAY_SURFACE:authority_inputs",
                "MISSING_DECISION_TIME_QUOTE",
                "MISSING_FORENSIC_BASELINE",
                "AMBIGUOUS_DEDUP",
                ...
                "REQUIRED_FABRICATION",
                "O5_ANCHORS_EMPTY_VALID",
            ],
        },
        "permitted_transformations": [
            ...
            },
            {
                "transform_id": "T-003",
                "description": (
                    "Deterministic classification into separate O3 / O4-A / "
                    "O4-B pipelines per freeze rules"
                ),
                "deterministic": True,
            },
            {
            ...
            },
            {
                "transform_id": "T-006",
                "description": (
                    "O5 locate-only enumeration of pre-existing sealed Tier-A "
                    "anchor manifests after acquisition-start"
                ),
                "deterministic": True,
            },
        ],
        "prohibited_substitutions": [
            ...
            "Converting unit-test fixtures or synthetic examples into empirical observations",
            ...
            "Outcome-conditioned rewrite of eligibility or sampling rules after inspection",
            "Mixing O4-A and O4-B payloads into one archive",
            "Synthesizing quotes, checkpoints, loss inputs, recovery state, or day_change from assumptions",
            "Using later market data as decision-time quotes",
            "Importing O4-B data into O4-A",
            "Filling missing surfaces from evaluator defaults",
            "Mutating or reusing QUAL-001 archive IDs",
            "Prospective instrumentation creating a new evidence population under this freeze",
        ],
        "expected_count_reconciliation": {
            "policy": (
                "Planned equations only; numeric empirical counts are "
                "NOT_APPLICABLE_BEFORE_SELECTION and must be filled only after "
                "acquisition-start selection; must reconcile or STOP"
            ),
            ...
            "formulas": {
                ...
                "identity": (
                    "source_count → window_eligible → deduplicated; "
                    "complete + excluded_by_reason must equal prior stage; "
                    "emitted ⊆ complete"
                ),
            },
            "pre_selection_numeric_counts": "NOT_APPLICABLE_BEFORE_SELECTION",
        },
        "target_archive_schemas": schemas,
        "archive_identity_rules": {
            "naming_pattern_o3": "O3-CAND-EVGAP-{capture_id}",
            "naming_pattern_o4a": "O4A-CAND-EVGAP-{capture_id}",
            "naming_pattern_o4b": "O4B-CAND-EVGAP-{capture_id}",
            "forbidden_archive_ids": [
                "O3-CAND-20260730T022316Z",
                "O4A-CAND-20260730T022316Z",
                "O4B-CAND-20260730T022316Z",
            ],
            "separate_pipelines": {
                "O4_A": "DECISION_TIME only",
                "O4_B": "FORENSIC only",
                "payload_merge": "FORBIDDEN",
            },
        },
        "storage_paths": {
        ...
        "qualification_criteria": {
            ...
            "required_proofs": [
                ...
                "no_prohibited_fabrication_of_gap_surfaces",
                "gap_completeness_bars_for_qualified_binding",
            ],
            "permitted_outcomes": [
                "QUALIFIED",
            ...
            "report_path_pattern": (
                "docs/design/evidence/dbox_evgap_acq_001/qualification/QUAL-*.md"
            ),
        },
        "broker_reads": {
            "policy": (
            ...
            "operations": [],
            "undeclared_broker_calls": "STOP_INCONCLUSIVE_OR_REQUIRE_AMENDMENT",
        },
        "tooling_identities": {
            "seal_script_path": seal_tool_path,
            "seal_script_sha256": file_sha(seal_tool_path),
            "schema_paths": [
                schemas["O3"]["path"],
                schemas["O4_A"]["path"],
                schemas["O4_B"]["path"],
            ...
            "content_tip_commit": content_tip_commit,
            "extraction_packaging_code": (
                "NOT_YET_PRESENT — may be developed under ACQ-AUTH-001 with "
                "synthetic fixtures only before acquisition-start; empirical "
        ...
        "stop_conditions": [
            ...
            "Decision-time cutoff cannot be reconstructed without invention",
            ...
            "Gap surface recovery would require prohibited inference",
            "Ambiguous lineage for any emitted row",
            "Mapping cannot be executed without guessing unresolved sources",
        ],
        "predetermined_inconclusive_handling": {
            "on_stop_condition": (
                "Close acquisition/construction INCONCLUSIVE or require "
                "superseding freeze + owner acknowledgment; do not invent "
                "observations"
            ),
            "on_empty_eligible_set": "INCONCLUSIVE — do not manufacture sample",
            "on_o5_empty_anchors": "INCONCLUSIVE with anchors:[] — valid",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--content-tip-commit",
        required=True,
        help="40-char git commit for content tip / tooling identity rebind",
    )
    parser.add_argument(
        "--write-unsealed",
        action="store_true",
        help="Write UNSEALED JSON (readiness may still FAIL if placeholders remain)",
    )
    parser.add_argument(
        "--seal",
        action="store_true",
        help="Seal after readiness PASS; writes SEALED JSON with countersignature",
    )
    parser.add_argument(
        "--readiness-only",
        action="store_true",
        help="Run readiness checks only; do not write",
    )
    args = parser.parse_args(argv)

    tip = args.content_tip_commit.strip().lower()
    if len(tip) != 40 or any(c not in "0123456789abcdef" for c in tip):
        print("FAIL: --content-tip-commit must be 40-char lowercase hex")
        return 1

    body = build_manifest_body(content_tip_commit=tip)
    errs = walk_check(body)
    if errs:
        print("READINESS FAIL:")
        for e in errs:
            print(" -", e)
        if args.write_unsealed:
            doc = {
                "schema_version": 1,
                "document_id": DOC_ID,
                "manifest_status": "UNSEALED",
                "manifest_body": body,
                "seal": None,
            }
            ...
            print("WROTE_UNSEALED_DESPITE_FAIL", UNSEALED_PATH.as_posix())
        return 1
    ...
    if args.write_unsealed:
        doc = {
            ...
            "manifest_status": "UNSEALED",
            "manifest_body": body,
            "provisional_body_sha256": digest,
            "seal": None,
            "note": (
                "UNSEALED — not authoritative for evidence access; seal + "
                "separate acquisition-start still required"
            ),
        }
        UNSEALED_PATH.write_text(
            json.dumps(doc, indent=2, ensure_ascii=False) + "\n",
        ...
        print("WROTE", UNSEALED_PATH.as_posix())
    ...
    if args.seal:
        ...
        loaded = json.loads(SEALED_PATH.read_text(encoding="utf-8"))
        assert body_sha256(loaded["manifest_body"]) == digest
        assert loaded["seal"]["body_sha256"] == digest
        print("WROTE", SEALED_PATH.as_posix())
        print("VERIFY_OK")
