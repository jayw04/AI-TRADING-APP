"""MR-002 — SAMPLE B-C1, the COUNTERSIGNED corrected disjoint sample (cardinality 100) (owner ruling 2026-07-14 §7-§10).
...
SEALED_BC1 = "/work/docs/implementation/evidence/mr_002/MR002_SampleBC1_Selection.json"
...
def main() -> int:  # noqa: PLR0915
    ...
    generator = sorted(rest, key=lambda i: fixture_hash(CORPUS[i]))   # THE FROZEN GENERATOR
    original_b = generator[:PROSPECTIVE_N]
    reserve = generator[PROSPECTIVE_N:]
    a_hashes = {fixture_hash(CORPUS[i]) for i in sample_a}

    # Reconstruct B-C1 by the countersigned frozen §6 rule: preserve non-colliding original-B
    # candidates; replace a content-hash-in-A / already-accepted candidate with the next eligible
    # reserve candidate (same exclusions). This is SELECTION reproduction, not re-selection after
    # outcomes.
    sample_b = []                                     # B-C1
    accepted = set()
    rp = 0
    ...
    def _next_reserve():
        nonlocal rp
        while rp < len(reserve):
            cand = reserve[rp]
            rp += 1
            h = fixture_hash(CORPUS[cand])
            if h not in a_hashes and h not in accepted:
                return cand, h
        return None

    for idx in original_b:
        h = fixture_hash(CORPUS[idx])
        if h in a_hashes or h in accepted:
            repl = _next_reserve()
            if repl is None:
                stops.append({"stop": "RESERVE_EXHAUSTED"})
                print("STOP: reserve exhausted before 100 unique disjoint hashes", file=sys.stderr)
                return 1
            sample_b.append(repl[0])
            accepted.add(repl[1])
        else:
            sample_b.append(idx)
            accepted.add(h)

    # ---- §7 PRE-PROOFS, before any repair: B-C1 must match the SEALED, COUNTERSIGNED selection ----
    with open(SEALED_BC1, encoding="utf-8") as fh:
        sealed = json.load(fh)
    bc1_hashes = [fixture_hash(CORPUS[i]) for i in sample_b]
    checks = {
        "matches_countersigned_selection": sample_b == sealed["sample_b_c1"],
        "content_hashes_match": bc1_hashes == sealed["sample_b_c1_content_hashes"],
        "cardinality_100": len(sample_b) == PROSPECTIVE_N,
        "unique_hashes_100": len(set(bc1_hashes)) == PROSPECTIVE_N,
        "zero_overlap_with_A": not (set(bc1_hashes) & a_hashes),
        "no_internal_duplicate": len(bc1_hashes) == len(set(bc1_hashes)),
    }
    print("--- §7 pre-proofs (B-C1 vs countersigned selection) ---")
    for k, v in checks.items():
        print(f"    [{'ok' if v else 'STOP'}] {k}")
    if not all(checks.values()):
        stops.append({"stop": "SAMPLE_BC1_SELECTION_MISMATCH", "checks": checks})
        print("STOP: B-C1 does not match the countersigned selection", file=sys.stderr)
        return 1
    print()

    # ---- exact repair + agreement certificates, per overlap --------------------------------------
    print(f"--- exact min-Linf repair (canonical) on {len(sample_b)} B-C1 overlaps ---")
    ...
    print("\n" + "=" * 74)
    print("  SAMPLE B-C1: " + ("PASS" if ok else "STOP FOR ADJUDICATION"))
    print("=" * 74)
    for s in stops[:10]:
        print(f"    {s['stop']}  instance={s.get('instance')}")

    doc = {
        "schema": "MR002_SampleBC1/v1",
        "authorization": "owner ruling 2026-07-14 §7-§10 + B-C1 cardinality-100 correction + countersign",
        ...
        "corpus_hash": ch,
        "sample_b_c1": sample_b,
        "sample_b_c1_content_hashes": [fixture_hash(CORPUS[i]) for i in sample_b],
        ...
        "sample_b_c1_pass": ok, "no_performance_computed": True,
        "validation_and_sealed_oos": "SEALED AND UNREAD",
    }
    blob = json.dumps(doc, indent=2, default=str)
    with open(f"{out_dir}/MR002_SampleBC1.json", "w", encoding="utf-8") as fh:
        fh.write(blob)
