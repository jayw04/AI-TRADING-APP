...
- **3A-6 Metrics + OOS-consumption** — `MetricRoleRegistry` (primary/secondary/diagnostic/integrity), metric spec, OOS-consumption protocol (stages O1-O5), null-model spec (binds the 5-trial ledger + dispersion rule; the dispersion artifact is produced at validation run time, not Phase 3A).
