...
CHECKPOINT = "b30cb16ba05d101cd903b158412d0cd62059ebfb"
...
SUPERSEDED = {
    "v3.0": {"identity": "73123895f947b19353f84a32505cf2e43b8236d23c792fe6adc2d33980b88333",
             "status": "ACCEPTED_THROUGH_PRE_ACCESS_READY, NOT EXECUTION-SPENDABLE",
             "why": "its s3_reader built an ambient-credential S3 client with no STS path, so the "
                    "first read would have been attempted as the qualified host role - which holds "
                    "an explicit Deny on the sealed bucket and must keep it. The package reached "
                    "S7 faithfully and could not cross it."},
...
GOVERNING_ONLY = (
    ...
    "MR002_Phase3B_SupplementalExecutionIdentity_v3.0.json",
...
QUALIFICATION = (
    ...
    "MR002_Phase3B_PreAccessReadyEvidence_v1.1.json",
    "MR002_Phase3B_LiveMaterializationObservation_v1.1.json",
)
...
def build() -> dict:
    evidence = json.loads(_blob(f"{GOV}/MR002_Phase3B_PreAccessReadyEvidence_v1.1.json"))
    ...
    return {
        ...
        "privilege_transition": {
            "reader_role_arn": "arn:aws:iam::219024422756:role/mr002-validation-reader",
            "session_name": "mr002-p3b-validation-v1",
            "topology": "qualified host role -> assume ONLY the governed reader role -> "
                        "validation-scoped S3 with OOS denied",
            "host_direct_sealed_access": "explicit Deny, retained permanently",
            "ambient_fallback": "none - assumption failure refuses before any S3 client is built",
            "arn_binding": "module constant; s3_reader takes no role_arn parameter, so a caller "
                           "cannot choose which identity reads the sealed store",
            "laziness": "no STS call until the first read, proven under --network none with "
                        "AWS_EC2_METADATA_DISABLED=true",
            "iam_latch_state_at_submission": "CLOSED - host->assume reader is explicitDeny; "
                                             "releasing it is a distinct owner-authorized event",
        },
        "known_limitation": evidence.get("known_limitation"),
        "authorization": {
...
def main() -> None:
    ...
    out = os.path.join(_HERE, "MR002_Phase3B_SupplementalExecutionIdentity_v3.1.json")
