# MR-002 — Pre-Registration **v1.0 — FROZEN** · Sector-Neutral Residual Reversion

**Date:** 2026-07-11 · **Owner:** Jay Wang · **Program ID:** MR-002 · **Registry:** **RUNNING**
· **Status:** ✅ **RESEARCH-DESIGN FREEZE SIGNED (owner, 2026-07-11).** Planning → **Running**. All
data gates MET; §8a fully populated; every artifact hashed and sealed.

> **IMMUTABLE RESEARCH INPUTS (owner governance, from the freeze signature).** The frozen universe ·
> identity registry · predecessor overrides · sector mapping · accepted exclusions · §8a window · V1
> anchor population · PIT-SIC population are **immutable**. Any change requires a **formally registered
> version increment** — never a silent modification of the v1.0 baseline. Mapping, identity, and
> coverage discussions are **closed** unless a newly discovered factual defect materially invalidates
> the frozen evidence.

> **Architectural lesson recorded (owner):** *a successful reconciliation is not proof that cached data
> are complete.* Truncated cache objects survived replay without generating fetch failures; atomic
> writes permanently eliminate that defect class. This belongs to the permanent architecture history
> and should be inherited by future ingestion systems.
...
*Prepared 2026-07-11. **SIGNED by the owner 2026-07-11 — Research-Design Freeze APPROVED; program is RUNNING.** Success is now judged by whether the implementation reproduces the frozen research design and whether validation + the single sealed-OOS run support the strategy — not by revisiting the data foundation.*
