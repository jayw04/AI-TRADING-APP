# MR-002 Validation / OOS Preregistration v1.0-RC1

**Status:** release candidate. All D-* decisions are frozen (`MR002_ValidationOOS_DecisionRecord_v1.0.json`,
...
`MR002_ValidationOOS_Preregistration_v1.0-RC1.json` (sha `399e3b53…`). **No validation or OOS data
was read** — only the session-date column of the pinned snapshot (owner-authorized metadata
extraction). RC1 promotes to **v1.0-final** on a single owner ratification: the realization-endpoint
convention (§seam). It authorizes nothing.
...
## Seam rule (correction 2; endpoint convention pending ratification)
...
> **Owner ratification RC1 → v1.0-final:** confirm the endpoint convention (recommended **6**,
> faithful to the frozen "next-open execution" applied to exits). The alternative (5) is fully
> computed so the choice is a one-line ratification, not a recomputation.
...
## To promote RC1 → v1.0-final

One owner ratification: the realization-endpoint convention (§seam; recommended 6). No other
deferred literal dates remain. Then Workstream B (evaluator build + synthetic qualification)
proceeds; validation/OOS stay sealed until the owner authorizes the validation opening.
