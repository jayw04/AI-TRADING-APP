...
| Field | Value |
|---|---|
| Document version | **v0.16 — Review-1 revision of the v0.15 profitability-first draft; FINAL-REVIEW DRAFT (Review 2 = final under the two-review-max policy)** |
| Version date | 2026-08-27 (Review 1 delivered the same day v0.15 was drafted; v0.15 was never accepted or landed) |
| Supersedes | **The v0.15 DRAFT** (`…_v0_15_Profitability_First.md`, never in Git custody) **and v0.14** (`docs/design/ATP/AlgoTraderPlus_v1_4_1_ImplementationPlan_v0_14.md`, the last version in Git). Because v0.15 never entered custody, **exactly one successor lands: v0.16.** Do not commit v0.15. |
| Intended canonical path | `docs/design/ATP/AlgoTraderPlus_v1_4_1_ImplementationPlan_v0_16.md` — **one file name, no suffix variants.** The v0.15 draft's `_Profitability_First` suffix and its §10A path (`…_v0_15.md`) named two different files for one document; that is the two-copy hazard the 2026-08-23 custody ruling exists to prevent. |
| Basis | Everything the v0.15 draft cites (retained verbatim below) **+ this Review-1 pass** (2026-08-27). No governed artifact, K-criterion, holdout, frozen design or owner ruling is changed by v0.16. |
| Repository | `github.com/jayw04/AI-TRADING-APP` |
| Scope | Unchanged from v0.15: **generate robust, deployable, net-profitable strategies as the sole program objective.** v0.16 keeps that rule and makes it *executable* — every active item now carries a named strategy, a mechanism, a dated or counted stop rule, and a next conversion gate (§0.4). |
| Version-control rule | **ONE CURRENT PLAN ONLY.** v0.16 is the sole current plan once accepted and landed. Operational/state changes update v0.16 **in place**, dated; a later substantive design revision receives the next number. v0.15 and earlier are history. |
| Governance stance | Planning document only. Governed program artifacts, frozen designs, sealed verdicts, owner rulings, ADRs and promotion gates control over this plan. **Research is instrumental; profit is the product.** |
| Review budget | v0.16 = **Review 1 applied**. The next pass is **Review 2 = FINAL**; after it the plan promotes to **v1.0** with no further review passes unless an execution-exposed inconsistency reopens it (standing two-review-max policy). |

---

## 0. CONTROLLING SECTION — Review-1 findings, amendments, and the executable queue *(v0.16, 2026-08-27)*

**Read this section first; it supersedes every lower section where they conflict.** The v0.15 draft's
controlling section (the `## State sync — 2026-08-27 15:39Z` block immediately below §0) is retained as
the record of what v0.15 proposed; §0 is what v0.16 adopts.

### 0.1 Review-1 verdict on the v0.15 draft

**Direction: ACCEPT.** The profitability admission test (§C of the v0.15 sync, §1.3.1 of the body) is the
right rule and is overdue — the platform has spent August producing excellent governance and no new
economic evidence. Demoting G10/K2, OPRA capture, the general feature library and standalone ops custody is
correct on the plan's own terms.

**Execution: REVISE before acceptance.** The draft states the rule but does not yet obey it. Five of its
nine active-queue rows fail the admission test they impose (no stop rule, no time-box, no conversion gate,
or no named strategy). The draft also declines to state the one conclusion its own §E/§F arithmetic forces
about the MDQ verdict, which would leave the October outcome looking post-hoc. And the document itself —
326 KB, 3,776 lines, nine in-place state syncs above the body, at least six lower sections that now assert
the opposite of the controlling section — has become an operational hazard of the kind the plan warns
about elsewhere ("a checklist inherited across versions accumulates items that were discharged but never
struck").

Twelve findings follow (§0.2), each with the amendment v0.16 applies (§0.3). Owner rulings that v0.16
needs are collected in §0.5. Everything else in the document is unchanged and retained.

### 0.2 Findings — R1 … R12

**R1 — The plan forecloses the MDQ GO verdict and does not say so.** *(Substantive; load-bearing.)*
Read the v0.15 draft's own sections together: K5 cannot contribute (signed §8.4); K4 is bound by a
non-waivable 250-event-day contract sitting at 4/250; K2 is NOT EVALUABLE unless G10 opens, and §F now
rules G10 will not open for MDQ reasons; K1 is "NOT EVALUABLE unless a pre-corpus governed provider or
predeclared defect declaration is *discovered*" and §B forbids creating one. That leaves **K3 plus
K6-if-a-stub-quote-is-captured** as the only two paths to the ratified ≥2 floor. K6 is event-contingent
on a 50-symbol, 60-second-cadence corpus. The honest expected disposition of G3 under v0.15 is therefore
**HOLD-with-stated-extension or STOP, not GO** — and the draft never states it. That silence matters:
in October, "we chose in August not to fund the paths that could have reached GO" is a legitimate
prospective owner decision; discovered then, it reads as a retention verdict shaped after the fact.
The profitability filter is right; it must be recorded *now* as a prospective ruling on expected
disposition (§0.5 ruling O-1), and the plan must state what the $99/month retention question turns on
if GO is not reached (R2).

**R2 — The subscription's economic decision is not the K1–K6 verdict, and the plan conflates them.**
The inventory report (v0.6 change 7) already separated *operational usefulness* (evidenced by the
account-7 program: GLD stub-spread absent on SIP, KMLM improved) from *permanent retention* (the K1–K6
decision). Under R1 the K-verdict is unlikely to be GO, so retention must be decided on the economic
question the profitability rule actually cares about: **does any named strategy's realized execution
economics depend on SIP?** Today the only strategy-direct evidence is CEE Session 001 (median SIP–IEX
shortfall difference **0.00 bps**, n=17, NOT EVALUABLE) and the account-7 executor's fail-closed Stage-B
spread gate. The plan needs a named retention test that is *not* K1–K6 (§0.3 A2).

**R3 — Five active-queue rows fail the §C admission test.** Applying steps 3 and 4 of the test to §D:

| Row | Missing |
|---|---|
| P0 LOW-001 S8.6 → B3a → paper activation | No pre-registered observation design: window length, metrics, what evidence would demote/keep/upgrade LOW-001's verdict B, and the RANK-001 interaction. "Measure realized return" is not a stop condition. |
| P0 finish-only #696 / K1 | "Time-box K1 provenance resolution" — no number. |
| P1 GAPPER data-source qualification | "Short bounded investigation" — no number, no owner. Also silent that the only non-purchase path (forward accrual of s/l channels) is capture-first work the same filter deprioritizes. |
| P1 SF1 NO-START census | "Zero strategy code; if population/PIT/OOS/power fail, stop" — good, but the census inputs share the **stale Sharadar `sep` spine (ends 2026-06-16)** and the Sharadar support escalation; a census over stale data mis-measures freshness/power. |
| P1 MOM-001 L1 | "No active work while population too small" — no numeric threshold; without one this row is un-reopenable and un-closable. |

**R4 — The LOW-001 P0 chain has no calendar and one contested fact.** LOW-001 fires Monday **10:32 ET**;
the next natural window is **2026-08-31**. The chain (deploy Amendment 8 → READY 6/6 → identity closure →
no-transition window → S8.6 1–12 → rollback restore → B3a on the same runtime) has "several owner
checkpoints and no deadline." If it does not close before the 08-31 fire, the first economic observation
slips a week per miss. Separately, the 08-26 §C text says "B3a is deployed with its production proof
unclosed," while the B3a repair record (2026-08-25) shows the notional-reference-price fix on branch
`fix/b3a-notional-reference-price`, **uncommitted, pending owner rulings** (fail-open on new positions is
platform-wide, gates Account-6 activation). One of those statements is stale. The plan must pin **which
B3a artifact** (branch/commit/PR) the P0 chain means, because "frozen B3a proof on the SAME runtime" is
unfalsifiable until the artifact is named.

**R5 — K1 OR-semantics correction in §B is itself slightly wrong.** Blocker 2 says the criterion "must
remain NOT EVALUABLE until both limbs are evaluable and neither passes." For a disjunction that is too
strong: one evaluable PASSing limb is sufficient for PASS regardless of the other. The correct rule:

```text
K1 = limb A OR limb B     (A: predeclared IEX gate-material defect corrected on SIP;
                           B: the alternate limb as frozen in the registration)

  any limb evaluable AND PASS                     -> PASS
  both limbs evaluable AND both FAIL              -> FAIL
  otherwise (any limb NOT EVALUABLE, no PASS)     -> NOT EVALUABLE
```

Three-valued OR: PASS dominates; FAIL requires both limbs evaluable; everything else is NOT EVALUABLE.
#696's tests should encode this truth table, not the sentence in §B.

**R6 — #696 blocker 1 (bypassable evidence boundary) needs an acceptance shape, not just a diagnosis.**
"`KResult` remains caller-constructible as evidentiary and `_mint_token` is externally callable" is the
right finding. v0.16 states the shape that closes it (§0.3 A6) so the fix is reviewable against a
specification rather than re-litigated at the next immutable review.

**R7 — GAPPER's realistic disposition under v0.15 is STOP-for-cycle, and the plan should say so.**
The DatasetOptions document already established that the five missing field classes are "buy the past
or wait for it" (~12 months of forward accrual at ~1 event-day/day, longer for environment diversity),
that class 4 (PIT locate/borrow) may be unobtainable at any price, and that a historical pull from
Alpaca is foreclosed by the subscription ruling. A "bounded feasibility" search therefore has exactly
two live outcomes: (a) a purchase decision with a named dataset identity, or (b) STOP-for-cycle with
forward accrual continuing passively. v0.16 names both and dates the decision.

**R8 — The document has outgrown the ONE-CURRENT-PLAN-with-in-place-syncs rule.** In-place state syncs
have made the file 326 KB. The controlling section now has to enumerate which lower sections it
contradicts ("calculators do not exist", "B-1 has not started", master gate table dated 2026-08-20,
§D's own P0 row marked superseded inside the same sync, Track 5 §5.4 vs §6.3, Track 3 §3.2). Every
future reader — human or model — pays that cost and risks acting on a stale paragraph. This is the
plan's own "phantom blocker" failure mode. The fix is structural (§0.3 A8) and is scheduled for v1.0,
not now, so that v0.16 remains a diff against v0.14 that a reviewer can actually read.

**R9 — The 08-27 partition will be the first sealed under a runtime whose self-report is known-false.**
§A.1.1 records the defect; §7.1 admissibility rests on *in-container blob hashes*, which are unaffected —
so admissibility holds. But the partition manifest carries `collector_version`/image provenance written
by that runtime. v0.16 requires the 08-27 seal record to carry one explicit line: *identity established
by running-container hash (authority order 1), not by the box's self-report, which was inaccurate on this
date.* Cheap now; expensive if someone later reconciles manifests against `DEPLOYED_BUILD_INFO.json`.

**R10 — "Outside the active queue" is right, but two items in it are latent P0 blockers.** The wrapper
and three systemd units that enforce the universe pin, free-space floor and slot grid exist **only on the
box, not in Git**. Amendment 8's deployment (P0 step 2) recreates the container and may touch that host
surface. If the deployment runbook rebuilds or moves those files, the first "READY 6/6" is produced by a
control whose identity is not in custody. v0.16 promotes **only the minimum blocking slice**: capture the
wrapper + unit SHAs into the Amendment-8 deployment record *before* the deploy (a read, not a PR), and
leave the custody PR in the OPS backlog as v0.15 intended.

**R11 — The SF1 sequencing exception is recommended in three places and ruled in none.** v0.3 item 10
made "start SF1 earlier" require an explicit owner exception; v0.15 §D, §6.3 item 11 and §8 item 3 all
recommend it. Until the owner signs it, Track 5 §5.4 ("G5 by default") governs and the census may not
start. v0.16 puts the ruling on the §0.5 list with a recommended text.

**R12 — Minor consistency defects** (fixed in place, no substance): master gate table header still reads
*State (2026-08-20)*; §10A names a different file path than the draft's filename; Track 1 heading still
says "awaiting the first admissible partition" (D0 was 2026-08-19); §6 lost the numbered execution-order
items 1–20 from v0.14 that other sections still cite by number (v0.16 restores a cross-reference note);
the v0.15 change summary item 5 says "once today's capture closes" — the dependency is the *deployment*,
not the capture, and the two are separate owner gates.

### 0.3 Amendments applied at v0.16 — A1 … A12

**A1 (R1) — Expected-disposition statement, recorded prospectively.** Added to the master gate table G3
row and to §0.5 as ruling O-1:

> Under the v0.15/v0.16 profitability filter, G10 will not be opened for MDQ reasons, K4's contract is
> not waivable and unmet, K5 cannot contribute, and K1 is evaluable only if a pre-corpus authority already
> exists. The ratified ≥2 floor is therefore reachable **only via K3 + K6 (event-contingent) or K3 + K1
> (if authority is found)**. The owner records now, before any K value is computed, that **HOLD with one
> stated extension or STOP is the expected G3 disposition**, that this follows from a prospective
> allocation decision and not from any K-criterion revision, and that such a disposition is **not a
> failure of the program**. No K definition, threshold, evaluability clause or the GO floor changes.

**A2 (R2) — Retention test that is not K1–K6.** New §0.4 row "ATP retention economic test":

> The $99/month decision at or before G3 is made on two strategy-direct facts, both already governed:
> **(i)** whether the account-7 executor's Stage-B spread gate *requires* SIP to avoid the IEX stub-quote
> false-reject class (the 2026-08-14 GLD incident) for any strategy that will trade in the next cycle; and
> **(ii)** whether CEE reaches N ≥ 50 qualifying fills with a median SIP–IEX shortfall difference whose
> 95% interval excludes 0 bps. If (i) is yes, retain regardless of the K verdict (execution-safety
> enabler for a named strategy). If (i) is no and (ii) is NOT EVALUABLE or ≈0, **cancel at the verdict
> date** unless a named strategy states a SIP dependency in its pre-registration. The MDQ K-verdict is
> reported as frozen and is *context* for this decision, not its trigger.

**A3 (R3) — Stop rules and time-boxes added to every active row** (see the §0.4 table). Specifically:
K1 provenance search **≤ 2 working sessions, ending 2026-09-04**; GAPPER data-source decision **by
2026-09-11** (two outcomes only, R7); MOM-001 L1 re-opens at **≥ 50 governed fills in the frozen Phase-A
universe within a rolling 60 sessions** (the same `N_min` already frozen for K5, reused rather than
invented); SF1 census **preceded by a Sharadar refresh-state check** — if `sep`/SF1 spines are stale past
the census's own freshness requirement, the census records **NOT EVALUABLE — STALE SPINE** and stops.

**A4 (R3, R4) — LOW-001 paper-observation pre-registration is a P0 deliverable, dated before the first
observed rebalance.** Added as P0 step 4a in §0.4:

> Before Account-6 activation, freeze in the LOW-001 track (not here): observation window (**≥ 13 weekly
> rebalances / one quarter**), metrics (net return vs frozen PIT-static reference, max drawdown, turnover,
> implementation shortfall vs decision price, conformance-check pass rate), the EVIDENCE_NOT_FEEDBACK
> firewall (no strategy change from paper results inside the window), and the disposition rule (KEEP as
> B/Diversifier · DEMOTE on conformance failure · no verdict *upgrade* from paper alone). RANK-001
> allocates only among independently validated strategies; paper observation does not change LOW-001's
> RANK standing. The 2026-08-12 → 08-21 pre-remediation window remains OBSERVATION_ONLY.

Calendar: the chain must either close **before the 2026-08-31 10:32 ET fire** or the owner records that
08-31 is skipped and 09-07 (Labor Day — **no session**; next fire **2026-09-14**) is the target. The plan
now carries that date rather than "no deadline." The B3a artifact identity is an **owner-supplied pin**
(§0.5 O-4); until pinned, the P0 chain's last step reads *B3a artifact: UNPINNED* and cannot be marked
done.

**A5 (R5) — K1 three-valued truth table** replaces the §B sentence as the acceptance criterion for #696
blocker 2. Tests must cover all nine limb-state combinations.

**A6 (R6) — Structural evidence boundary, acceptance shape for #696 blocker 1:**

```text
1. KResult has NO public constructor path that yields evidentiary=True.
   Evidentiary results are produced ONLY by an internal evaluate() that
   (a) takes an AdmissibilityReceipt (partition id + manifest sha256 + §7.1 verdict + receipt hash),
   (b) mints the token inside the module from that receipt hash + code identity, and
   (c) is the only call site of the token minter (enforced by an import-boundary test,
       same pattern as the §4.5 no-broker invariant).
2. Any KResult constructed outside evaluate() is evidentiary=False and carries
   provenance="CALLER_CONSTRUCTED"; the verdict aggregator refuses it.
3. Provider/defect-list injection (blocker 3): an injected provider is accepted only if it
   carries a governed-artifact envelope whose artifact_sha256 matches an entry in a
   pre-corpus registry file committed BEFORE D0 (2026-08-19). Absent that match the limb
   is NOT EVALUABLE — never FAIL, never PASS.
4. Threshold regressions (blocker 4): exact-equality cases at 10.0% (K1) and 50.0% (K3)
   in both directions, with the frozen comparison operator named in the test.
```

**A7 (R7) — GAPPER disposition pre-declared.** §0.4 row now reads: decision by 2026-09-11 with exactly
two outcomes — **PURCHASE** (owner binds `source_vendor` to one dataset identity + coverage period +
field set + PIT semantics; census re-runs) or **STOP-FOR-CYCLE** (forward accrual continues passively;
no preparation work; re-entry when census ≥ 250 event-days *or* a purchase decision). "Keep investigating"
is not an outcome.

**A8 (R8) — v1.0 restructure, scheduled and scoped.** At v1.0 (after Review 2), the plan splits into
**(a)** a current plan ≤ ~40 KB carrying §§0–11 with state syncs *summarized* and **(b)**
`…_History_v0_3_to_v0_16.md`, the retained verbatim state-sync record, hash-pinned from the v0.16 blob.
History is never rewritten; it is *relocated* with its bytes preserved. Until v1.0, v0.16 keeps the full
body so this revision remains reviewable as a diff against v0.14.

**A9 (R9) — 08-27 seal-record line** (operational instruction, §0.4 P0 row 1): the seal/state-sync entry
for 2026-08-27 must state that identity was established by running-container hash (authority order 1)
and that the host self-report was inaccurate on this date by design.

**A10 (R10) — Minimum blocking slice of wrapper/unit custody.** Before the Amendment-8 deploy: read and
record SHA-256 of `/opt/workbench/mdq/mdq_run.sh` and the three unit files into the deployment record;
after the deploy, re-read and confirm unchanged (or record the delta as part of the deployment
identity). The custody PR itself stays in the OPS backlog.

**A11 (R11) — SF1 exception placed on the ruling list** (§0.5 O-3) with recommended text.

**A12 (R12) — In-place consistency fixes:** gate-table header → *State (2026-08-27; see §0)*; §10A path
→ `…_v0_16.md`; Track 1 heading annotated; §6 cross-reference note added; change-summary item 5 wording
corrected in §0 (the retained v0.15 text below is left as written).

### 0.4 Executable queue — v0.16 *(replaces v0.15 §D)*

Every row satisfies the §1.3.1 admission test: strategy/decision · mechanism · stop rule · next gate.

| # | Work | Class | Serves | Stop rule / time-box | Next gate |
|---|---|---|---|---|---|
| **P0-1** | Close the 08-27 capture under the deployed five-gate control; seal record carries the A9 identity line | REQUIRED ENABLER | MDQ corpus (context for A2) | Terminal 395/395 + EOD + freeze→verify→mirror + independent S3 check; else NON-EVENT, no salvage | — |
| **P0-2** | Owner deployment ruling for Amendment 8; record wrapper/unit SHAs (A10); deploy exact SHA; first governing **READY 6/6**; close production deployment identity | REQUIRED ENABLER | **LOW-001** paper activation | Must close before **2026-08-31 10:32 ET** or owner records the slip to **2026-09-14**; no embellishment beyond identity proof | LOW-001 S8.6 |
| **P0-3** | Fresh no-transition window → S8.6 checks 1–12 → rollback restore iff 12/12 → **B3a (artifact PINNED per O-4)** on the same runtime | STRATEGY-DIRECT | **LOW-001** | Any check fails ⇒ record, no substitution, no re-run in the same window | Account-6 activation |
| **P0-4** | Freeze LOW-001 paper-observation design (A4) **before** activation; then observe | STRATEGY-DIRECT | **LOW-001** | ≥ 13 rebalances; KEEP/DEMOTE only; no upgrade from paper; EVIDENCE_NOT_FEEDBACK | Owner disposition at window end |
| **P0-5 finish-only** | Correct #696 per A5/A6; land K3 + K1 frame after immutable review + CI | REQUIRED ENABLER | ATP retention decision (A2) | Provenance-only K1 authority search **≤ 2 sessions, ends 2026-09-04**; none found ⇒ **K1 = NOT EVALUABLE**, recorded, closed | G3 verdict artifact |
| **P1-1** | SF1 NO-START census (needs O-3) — preceded by Sharadar refresh-state check | STRATEGY-DIRECT | New PIT fundamental-change hypothesis | 4–6 h; stale spine ⇒ NOT EVALUABLE—STALE SPINE; population/PIT/OOS/power fail ⇒ STOP | One pre-registration or STOP |
| **P1-2** | GAPPER data-source decision (A7) | STRATEGY-DIRECT ENABLER | **GAPPER** | Decision **by 2026-09-11**: PURCHASE or STOP-FOR-CYCLE; nothing else | Census re-run (PURCHASE) or passive accrual (STOP) |
| **P1-3** | MOM-001 L1 execution enhancement | STRATEGY-DIRECT | **MOM-001** | Reopens at ≥ 50 governed fills / rolling 60 sessions in the Phase-A universe; otherwise HOLD, no manufactured observations | L1 overlay pre-registration |
| **P1-4** | ATP retention economic test (A2) | STRATEGY-DIRECT decision | Any strategy with a stated SIP dependency | Decided at or before G3 verdict date on facts (i)/(ii); K verdict is context | Keep/cancel |
...
| **P2 cond.** | SIP-native MOM-LIQ / SIP-CONT / SIP-LSR | STRATEGY-DIRECT | New alpha | Start only if A2(ii) shows material SIP information beyond data quality | Pre-registration |
| **Future cond.** | OPRA overlays | STRATEGY-DIRECT | Named strategy only | No capture-first work | G8 → G9 |

**Removed from the active queue** — unchanged from v0.15 §E, plus A10's clarification that wrapper/unit
custody is a *read* before P0-2 and a *PR* only in the OPS backlog.

### 0.5 Owner rulings v0.16 requires *(collected; none pre-empted)*

| # | Ruling | Recommended text |
|---|---|---|
| **O-1** | Expected G3 disposition under the profitability filter (A1) | *"Recorded 2026-08-27, before any K value is computed: HOLD-with-one-stated-extension or STOP is the expected G3 disposition. This follows from the prospective decision not to open G10 or fund K4 data acquisition for MDQ reasons, changes no K definition, and is not a program failure."* |
| **O-2** | ATP retention test (A2) adopted as the economic keep/cancel rule | *"Retention is decided on facts (i)/(ii) of §0.3 A2; the K1–K6 verdict is reported as frozen and is context."* |
| **O-3** | SF1 sequencing exception (R11) | *"Track 5 §5.4's G5-default is waived once; the NO-START census may run before GAPPER disposition. Zero strategy code; census output is NON-VERDICT."* |
| **O-4** | B3a artifact pin (R4) | Owner supplies branch/commit/PR of the B3a proof artifact the P0 chain executes; until supplied, P0-3 reads UNPINNED. |
| **O-5** | LOW-001 08-31 target or recorded slip (A4) | *"Target = close P0-2/P0-3 before 2026-08-31 10:32 ET; if not, slip is recorded now to 2026-09-14 and the 08-31/09-07 windows are not partially consumed."* |
| **O-6** | Amendment 8 deployment authority | Already listed in v0.15 §8 item 1; carried, with A10 as a pre-deploy read. |
| **O-7** | Accept A8's v1.0 restructure shape | Split at v1.0 into current plan + hash-pinned history file; bytes preserved, never rewritten. |

### 0.6 Change summary — v0.16 (Review-1 revision, 2026-08-27)

1. **Expected MDQ disposition made explicit and prospective** (A1/O-1); retention decision separated from
   the K verdict and given a strategy-direct test (A2/O-2).
2. **Every active-queue row now carries a stop rule, time-box or numeric threshold** (A3); the queue table
   in §0.4 replaces v0.15 §D.
3. **LOW-001 P0 chain given a calendar (08-31 → 09-14), a pre-registered paper-observation design, and a
   B3a artifact pin requirement** (A4/O-4/O-5).
4. **#696 acceptance criteria specified**: three-valued K1 truth table (A5) and the structural
   evidence-boundary shape (A6).
5. **GAPPER decision pre-declared as PURCHASE or STOP-FOR-CYCLE by 2026-09-11** (A7).
6. **Document-size hazard acknowledged; v1.0 restructure scoped** (A8/O-7).
7. **08-27 seal identity line and wrapper/unit SHA read added as operational instructions** (A9, A10).
8. **SF1 exception moved from "recommended" to a ruling with text** (A11/O-3).
9. **In-place consistency fixes** (A12).
10. **Custody:** v0.15 draft is not committed; v0.16 is the single successor to v0.14 in Git.

---

> **HISTORICAL — v0.15 DRAFT (2026-08-27, never accepted).** Everything below this line is the v0.15
> draft retained verbatim as the record of what was proposed and what was true at 15:39Z on 2026-08-27.
> The v0.15 header table has been removed (its content is superseded by the v0.16 header above); the
> first retained block is v0.15's own controlling section. Where any lower section conflicts with §0,
> **§0 governs.**

## State sync — 2026-08-27 15:39Z — profitability-first realignment + B-1 current state

**This is the controlling current-state section for v0.15.** The earlier 2026-08-27 v0.14 sync is retained
below as historical evidence of what was true before #695 merged and before B-1 was implemented. Where a
lower section says **"B-1 has not started"**, **"calculators do not exist"**, or treats **G10/K2 as a P0
calendar lever**, this section supersedes it.

### A. Repository / deployment / capture state

- `main` = **`3f32c75b1053f8181f98ddf51bbc473364ffd34c`** (#695 squash, 2026-08-27T14:56:31Z).
- #693 / Amendment 8 remains **MERGED** at squash `47715b4e…`; **deployment is still a separate owner gate
  and remains HOLD**. Six-gate governance begins only after that repaired control is actually deployed.
- The 2026-08-27 acquisition is still **RUNNING under the deployed pre-repair five-gate control**. It has
  not earned governed-partition status yet; terminal completeness, EOD, freeze→verify→mirror and
  independent S3 custody remain required.
- LOW-001 Run 2 / S8.6 / B3a remain HOLD until the deployment-identity repair is deployed and its
  production identity proof closes.

### B. B-1 current state — implemented candidate, not accepted yet

Draft PR **#696**, head **`2be09f477da2f0b9918d6dfad68cc6207deecc34`**, base `3f32c75b…`, one
commit / 10 files, now contains `app/research/mdq_eval/`, the evaluation CLI, and tests.

**Accepted direction:**
- K3 implements the frozen union-grid metric, half-open 04:00–16:00 ET window, zero-IEX-missing
  ⇒ NOT EVALUABLE, absent-file refusal, timezone normalization, and diagnostic-only row-count treatment.
- K1 correctly refuses to invent a SCAN/GAPPER decision provider or a post-hoc "predeclared" defect list.
- Per-partition §7.1 admissibility remains a prerequisite for evidentiary K evaluation.

**Immutable-review HOLD before #696 can leave draft:**
1. the claimed structural evidence boundary is bypassable because `KResult` remains caller-constructible
   as evidentiary and `_mint_token` is externally callable;
2. K1 OR semantics are wrong for `FAIL OR NOT_EVALUABLE` — the criterion must remain NOT EVALUABLE until
   both limbs are evaluable and neither passes;
3. caller-injected providers/defect lists cannot acquire evidentiary authority merely by being supplied;
4. the exact frozen 10% K1 and 50% K3 threshold boundaries need true equality regressions.

**Economic implication:** K1 is **currently NOT EVALUABLE unless a pre-corpus governed provider/binding or
pre-existing defect declaration is discovered.** Do not create either now. K3 is the only currently
computable K contributor. This is useful for the subscription/market-data decision, but it is not itself
a profitable strategy.

### C. Owner objective — one purpose: produce profitable strategies

All active work must pass this admission test:

```text
1. Name the strategy or economic decision the task serves.
2. State the mechanism by which it can improve net P&L, risk-adjusted return,
   capacity, execution cost, drawdown, or time-to-paper validation.
3. State the evidence/stop condition before work starts.
4. State the next conversion gate:
      observation -> frozen hypothesis/overlay -> governed validation
                  -> paper candidate -> promotion decision.
5. If no named conversion target exists, DEFER / MOVE TO OPS / STOP.
```

There are only two active classes:

- **STRATEGY-DIRECT** — generates, validates, improves, or promotes a named economic mechanism.
- **REQUIRED ENABLER** — the minimum engineering/data/governance work without which a strategy-direct
  step cannot execute safely or produce admissible evidence.

"Useful platform work", "more complete research", "better custody", "more features", and "we already have
the data" are **not sufficient reasons** to consume strategy-program time.

### D. Profitability-first active queue

| Priority | Work | Class | Economic conversion / stop rule |
|---|---|---|---|
| **P0** | **Close the 08-27 capture, then separately authorize/deploy Amendment 8 and close production deployment identity** | REQUIRED ENABLER | Directly unlocks LOW-001 S8.6/B3a and governed paper activation. Stop once production identity is proven; no extra platform embellishment. |
| **P0** | **LOW-001 S8.6 → B3a → governed paper activation / economic observation** | STRATEGY-DIRECT | Existing frozen strategy becomes executable; measure realized return, drawdown, turnover and conformance. This is a nearer path to real strategy evidence than more platform research. |
| **P0 / finish-only** | **Correct #696 and land K3/K1 frame** | REQUIRED ENABLER | Finish the already-built decision machinery once. Time-box K1 provenance resolution; if no unique pre-corpus authority exists, record K1 NOT EVALUABLE rather than funding definition repair. |
| **P1** | **GAPPER data-source qualification — bounded feasibility only** | STRATEGY-DIRECT ENABLER | Current 4/250 event-days cannot support Stage 0. Identify one qualified path to the required corpus; if no credible path exists within a short bounded investigation, stop GAPPER for this cycle. |
| **P1 — owner decision recommended** | **Pull forward the SF1 NO-START census** | STRATEGY-DIRECT | Cheap feasibility test for a distinct PIT fundamental-change alpha mechanism while GAPPER is data-blocked. Zero strategy code; if population/PIT/OOS/power fail, stop immediately. |
| **P1 when population exists** | **MOM-001 L1 execution enhancement** | STRATEGY-DIRECT | Existing alpha first: lower shortfall / stale-spread rejection / execution loss without changing ranking. No active work while governed fill population is too small. |
| **P2** | **MOM-CAND-001 / RSI-REV-001 prospective hypotheses** | STRATEGY-DIRECT | New alpha only after mechanism, cost model, falsification rule and untouched prospective test are frozen. |
| **P2 conditional** | **SIP-native MOM-LIQ / SIP-CONT / liquidity-shock work** | STRATEGY-DIRECT | Start only if current MDQ/CEE evidence shows SIP has material economic information beyond data quality. |
| **Future conditional** | **OPRA risk overlays / options-derived strategies** | STRATEGY-DIRECT | No capture/build merely because entitlement exists; require a named strategy interaction and a predeclared corpus sufficiency target first. |

### E. Work removed from the active strategy queue

The following may happen naturally or be carried in an OPS/product backlog, but they receive **no
dedicated strategy-program engineering session** unless they become a named blocker for a P0/P1 strategy:

- wrapper/systemd source custody as a standalone task;
- first scheduled SQLite-backup file proof;
- 90-day JSON-prune survival proof;
- branch hygiene / PR-history cleanup / documentation-location decisions;
- general DISC-MDQ feature-library construction;
- repeated DISC census as manual research work — snapshots may continue naturally and the census may be
  recomputed cheaply, but no feature investment follows an empty population;
- `RANGE-SIP-OBS-001` without a prospectively named new economic mechanism;
- local live-cache ADR/build before a specific L1/L2 strategy migration requires it;
- OPRA-CAP-001 capture before a named options/risk strategy requires it;
- **G10/K2 Phase-B implementation solely to rescue the MDQ GO floor.**

### F. G10/K2 — important qualification criterion, not a strategy priority

K2's 20-session / ≥250-symbol streaming reliability test answers whether a broad streaming acquisition
capability is reliable. **It does not itself create alpha, reduce execution cost for an existing strategy,
or produce a paper candidate.**

Therefore v0.15 changes the planning treatment:

> **Do not open G10 merely because K1 may be NOT EVALUABLE or because the MDQ keep/cancel floor becomes
> harder to satisfy.** Open G10 only when a named strategy/overlay requires broad streaming and states the
> economic mechanism it will test. Otherwise K2 remains NOT EVALUABLE for this MDQ cycle.

The 2026-09-21 last-start date remains a factual deadline **if** G10 is later justified economically; it
is no longer a reason by itself to allocate engineering effort.

### G. DISC / Opportunity — product surface is not the strategy

Opportunity Watchlist v1.0 is useful product infrastructure, but watchlist completeness, history polish,
backup proof and prune proof are not alpha. The only strategy-valid conversion is:

```text
candidate observation
  -> documented mechanism
  -> discovery-ledger citation / multiple-comparison disclosure
  -> prospective pre-registration
  -> untouched prospective validation
  -> paper strategy candidate
```

Current DISC-MDQ population remains too narrow for general feature work. MOM-CORE × MDQ may reopen as a
bounded execution-quality observation only if its expected decision value exceeds its cost; empty
MOM-NEAR / OVERSOLD / GAP branches receive no feature engineering.
...
⚠ **The forward plan below is ADJUSTED, and the adjustment is the point of this sync.** See §D.
⚠ **Afternoon supplement (§G–§I):** Watchlist v1.0 closeout, laptop-store retirement, and the
restated remaining-task order. **Does not** claim an 08-27 partition, **does not** deploy Amendment 8,
and **did not, at that earlier sync point, start B-1; superseded later the same day by the v0.15/#696 state above.**
...
| 3. **Natural start (causality)** | timer `LastTriggerUSec` == service `ExecMainStartTimestamp` == **`09:25:01 EDT`**; `TriggeredBy=mdq-sample.timer`; `InvocationID 64dc31fe3fbd4bf3b330b92642b55f74` |

⭐⭐ **Proof 3 must establish CAUSALITY, not presence.** "A collector is running" does not exclude a
hand-start; the timer-side `LastTriggerUSec` matching the service-side start timestamp to the second
does. The joint timer+service journal shows the activation with no preceding manual invocation.

Both feeds writing at 13:27Z (150 rows each = 3 cycles × 50), `ALERTS_TODAY = 0`.

⛔ **Not evidence yet.** The day earns partition status only from the terminal completeness stamp, the
16:30 ET EOD write, the 16:45 ET freeze→verify→mirror, and **independent** S3 custody verification.

⚠⚠ **Today's READY was produced by the DEPLOYED FIVE-GATE control** (`2ad345b8…`). The six-gate repair
is not deployed, so today is governed by the five-gate standard — correctly. It follows that today's
READY means *the five readiness predicates pass* and **nothing whatsoever about deployment identity**:
...
written with after the runtime moves underneath it, so three mutually agreeing assertions are not three
independent witnesses. Production falsified this on 2026-08-26.

**Two checks, deliberately unequal authority — neither substitutes for the other:**
...
| Host-side MDQ **Gate 6** | **independent attestation** | the build marker, deploy manifest and running runtime reconcile |

⛔ The startup gate is **integrity enforcement, never provenance** — a wrong image passes it, describing
itself correctly. ⛔ Gate 6 proves those artifacts *reconcile*; it does **not** prove the commit was
organizationally *approved*. Approval remains the separate governance authorization that selected the
immutable SHA.

### B.1 ⭐ THIS CHANGES THE MDQ READINESS STANDARD — prospectively, at deployment

When the repair is deployed, the standard moves from **five-gate preflight + natural timer** to
**six-gate preflight + natural timer**, and a Gate-6 failure means **NOT READY**. The three-proof chain
keeps its shape; each readiness proof becomes 6/6.

⛔ **There is no environment switch that makes Gate 6 count.** A default-off integrity gate is silent
degradation — the repair could deploy while someone forgets the flag, and the control would go on
declaring READY on five gates. **The boundary is enforced by deployment, not by a flag:** a box running
the old script keeps the five-gate standard until the new control is actually on it.

⇒ **Operational consequence for this plan:** every future MDQ readiness proof must state *which control
version produced it*. A bare "READY 5/5" after the repair is deployed would be a stale-control reading,
which is the same class of error as a stale pin reading "unchanged".

### C. Corpus and the calendar — recomputed today, not carried forward

**Corpus: five sealed governed days — 08-19, 08-20, 08-21, 08-25, 08-26** (08-27 pending).

| Milestone | Date | Sessions remaining from 2026-08-27 |
|---|---|---:|
| **Last possible K2 measurement start** | **2026-09-21** | **17** |
| Last session before the period holdout opens | 2026-10-05 | 27 |
| Last usable session in the review window | 2026-10-16 | 36 |
| Review window closes | 2026-10-18 | — |

⚠ **17 sessions to the K2 deadline.** The G10 owner decision must land well before it, because 09-21
carries zero implementation or qualification slack.

### D. ⭐⭐ ADJUSTED FORWARD PLAN — what the last two days actually changed

**What was planned on 08-26:** seal the partition → state sync → **B-1 build the K calculators (P0)** →
runtime self-report repair → wrapper custody.

**What actually happened:** the partition sealed, and the entire window since was consumed by the
self-report repair — legitimately, under owner authorization, and it grew from "extend one module" into
a 21-file governed Amendment because production had falsified the declaration-only model. **At that earlier sync point B-1 had not started. Superseded later 2026-08-27: draft #696 now contains the implementation candidate.**
...
| Priority | Item | Why |
|---|---|---|
| **P0 — superseded by v0.15** | **B-1: #696 implementation candidate exists; correct and finish only** | K3 is implemented; K1 is presently governance-input constrained. See the controlling v0.15 state sync above. |
| **DEFERRED by v0.15 profitability filter** | **G10/K2** | The date remains factual, but no work is justified unless a named profitable-strategy hypothesis requires broad streaming. |
...
### E. Verdict reachability — unchanged, restated because it drives §D

The ratified floor is **≥2 of K1–K6 both evaluable AND PASS**.

| Path | Can contribute? | Basis |
|---|---|---|
| **K1**, **K3** | K3 computable; K1 currently NOT EVALUABLE absent governed inputs | #696 implements K3 and a K1 frame; immutable corrections remain before merge. |
| K2 | only via a timely G10 / Phase-B run | NOT EVALUABLE without G10 |
| K4 | only via a conforming in-window Stage-0 run | 250-event-day contract, **not waivable** |
| K5 | **no**, even on a mechanical PASS | signed §8.4.2 item 4 |
| K6 | only if a qualifying occurrence is observed | event-contingent |

> Under the current state, if either K1 or K3 fails or is NOT EVALUABLE, GO becomes unreachable **unless
> at least one presently non-contributing path first becomes legitimately contributing in-window**. No
> such path may be created retroactively after its evidentiary window closes.

⚠ **Ruling still needed before G10 is decided:** any 20-session K2 window ending 10-16 puts **9 of its
20 sessions inside the period holdout**. Registration Ruling 4 scopes the predicate to
`exploratory_access_allowed` / discovery, so verdict computation of a frozen K criterion should be
admissible — but if that were inverted, only 11 sessions remain, K2 is structurally impossible, and the
lever is already dead.

### F. ⚠ CI artifact classification — record it, do not re-derive it

`gh pr checks` shows draft-time runs permanently attached to a head. On #693:

- **1657** — draft-time fail-closed artifact on `50be5921…`. No implementation test job executed; the
  Python CI Gate failed because prerequisite jobs were skipped while the PR was draft. **Not evidence of
  a defect.** Authoritative ready-state evidence is **run 1659 on the same head, which SUCCEEDED.**
- **1655** — the same artifact on the superseded head `d38da120…`; **1656** was that head's *real*
  failure (an obsolete hermetic deploy fixture, repaired in the successor).

⛔ Never read "red on the same SHA" as contradictory without first checking **whether the test jobs
actually ran**.

### G. Opportunity Watchlist v1.0 — Phase 1.1 **product closeout** *(missing from the morning sync)*

`docs/design/Opportunity Page/TradingWorkbench_Opportunity_Page_Watchlist_Design_v1_0.md` — **v1.0**,
2026-08-26, **MERGED `#691` / squash `08e4cc0`**. This is a **product/ops closeout**, not a strategy
pre-registration and not an authorization to trade or to influence LOW-001.

| Item | State |
|---|---|
| Phase 1.1 product functionality | **COMPLETE** |
| History filters / listing polish | **CLOSED / live** (`#669` / `e7f56c3`; `#681` / `e58ee77` display-only MOM-001 listing) |
| SQLite backup *inclusion* | **CONFIRMED** (WAL-safe one-shot 2026-08-26) |
| `#690` scheduled-backup *mechanism* | **MERGED** (`5c5969e`) and **already rebuilt into the 08-26 backend image** (08-26 §A.1 seam). Remaining proof is the **first 02:00 ET dated file written by the job**, not another one-shot |
| `SCREEN_VERSION` | **`DISC-001-WATCHLIST` / `v0.3.0` — unchanged** |
| 90-day JSON prune | **FUTURE NATURAL PROOF** (~2026-11-12 from earliest snapshot 2026-08-14) |
| Strategy 8 | **IDLE / 1.0.3** — do not start from this closeout |

⛔ Still out of this closeout (unchanged): MDQ on the Opportunity surface · checkpoint persistence ·
`SCREEN_VERSION` bump · gate retune · order-path / broker / risk expansion · `EVIDENCE_NOT_FEEDBACK`
fills filter · firing `disc001_watchlist_snapshot` except by owner request.

This does **not** reopen the DISC-MDQ feature-library HOLD, does **not** change frozen admission, and
does **not** make Watchlist output an MDQ or LOW-001 input.

### H. Laptop standby trading-data retirement *(2026-08-27, travel prep)*

The live book remains **AWS-only** (`ec2-paper` / `i-084f47fe4e69192e9`). Unique laptop-only trading and
research stores were archived, size-checked, then deleted locally. They were **not** restored over
`/opt/workbench/data/workbench.sqlite`.

| | |
|---|---|
| Archive prefix | `s3://workbench-backups-219024422756/artifacts/archive/laptop-standby-20260827/` |
| Inventory | `…/INVENTORY.json` (37 objects, 3.68 GB, Version IDs + SHA-256) |
| Live DB on the box | **untouched** — 27 MB, 10 strategies / 7 accounts / 1,592 orders, scheduler `ec2-paper` armed |
| Local SQLite / DuckDB / `bars_cache` | **gone** from the laptop data dirs |
| `.mr002out` + ingest `logs/` | **deleted locally** (research scratch / regenerable logs; not re-uploaded) |

⭐ **Load-bearing for SEC-001 successors, not for MDQ:** rider-2 recorded that the only artifact with a
pre-2024 PIT-200 cross-section was the ungoverned laptop `factor_data_full.refresh.duckdb` (and it had
**no `permaticker`**). That file is **no longer on the laptop**. The archived object is
`…/apps/backend/data/factor_data_full.refresh.duckdb`, pinned by Version ID + SHA-256 in that prefix's
`INVENTORY.json`. ⛔ It remains **ungoverned**. Do not treat the archive as a V3.1 input without a
separate owner qualification. The live ranking store on the box is still the 45 MB
`factor_data.duckdb` (production operating store, not a historical PIT-200).

### I. Remaining ATP / MDQ tasks — current order *(restated 2026-08-27 afternoon)*

Subordinate list: `docs/design/ATP/ATP_MDQ001_OpenTaskList_2026-08-26.md` (revised in place 2026-08-27).
**No new authority.** SEC-001 V3.1 stays **out of this list** (08-26 §D).

**Historical afternoon checkpoint:** 08-27 had **not** earned a partition and Amendment 8 was **merged, not deployed**. **B-1 status is superseded by the later v0.15 sync: draft #696 now contains the implementation candidate.**

```text
P0 — DEADLINE-BEARING, start now, in parallel with the deployment chain
  B-1   SUPERSEDED: #696 now implements K3 + K1 frame; correct immutable-review blockers, then finish
  B-5   DEFERRED by v0.15 profitability filter; do not open G10 solely to rescue the MDQ floor
  K4    measure whether the 250-event-day gap has ANY realistic closure path
        (contract NOT waivable — measure, do not "open")
  B-6b  rule holdout-scope vs K2 measurement BEFORE G10 is decided
  C-3   keep accumulating DISC snapshots / repeating the DISC-MDQ census
        (the only sanctioned DISC-MDQ activity; library remains HELD)
...
P2
  D-1   wrapper + systemd units into versioned custody
  first 02:00 ET scheduled SQLite backup file  (Watchlist v1.0 remaining ops proof)
  90-day JSON prune survival                   (~2026-11-12; do not invent a prune)

HOLD
  LOW-001 Run 2 / S8.6 / B3a production proof  (conjunctive; downstream of P1)
  broad DISC-MDQ feature library               (C-2; census still GAP 0 / MOM-CORE 5)
  C-1 DISC-001 holdout-honouring ruling        (owner; blocks DISC-MDQ direction)
```

⛔ Do not wait for P1 to start B-1. ⛔ Do not treat today's five-gate READY as a six-gate READY.
⛔ Do not start strategy 8 from the Watchlist closeout. ⛔ Do not execute GAPPER Stage 0 against the
present corpus (data sufficiency still fails the §3.1 contract).

**GAPPER `#670` — historical closeout, not a 08-23 rewrite.** The 08-23 same-day sequence still
reads `PR #670 open` because that is what was true that evening. Closure: **MERGED**
2026-08-24T00:45:57Z as squash `4c4a2b1` (`4c4a2b1089bbffa9d71022ebb9def1bd7431d3a6`) — `/v2`
write-time provenance + quarantine of `repair_premarket_gate_provenance.py`. Stage 0 remains
blocked on the §3.1 data-sufficiency contract regardless.
...
## Change summary — v0.15 (profitability-first execution realignment, 2026-08-27)

1. **One objective is now explicit and controlling:** produce robust, deployable, net-profitable
   strategies. Every active task must be STRATEGY-DIRECT or the minimum REQUIRED ENABLER.
2. **B-1 state corrected:** #696 exists and K3/K1 machinery is implemented as a draft candidate; it is
   not accepted yet. The current immutable-review blockers are recorded.
3. **K1 is no longer treated as automatically reachable:** absent a pre-corpus governed provider or
   defect declaration, record NOT EVALUABLE rather than creating authority after evidence exists.
4. **G10/K2 is demoted from P0 calendar lever to conditional capability.** Do not build broad streaming
   solely to rescue an MDQ retention verdict.
5. **LOW-001 activation chain becomes the highest-value immediate enabler** once today's capture closes:
   deploy identity repair → six-gate proof → S8.6 → B3a → governed paper economics.
6. **SF1 NO-START census is recommended for pull-forward** because it directly tests a new alpha path at
   low cost while GAPPER is data-blocked.
7. **GAPPER work is time-boxed to data-source feasibility.** No indefinite preparation against 4/250
   event-days.
8. **Platform/product/ops work is removed from the active strategy queue** unless it becomes a concrete
   blocker for a named strategy.
9. **DISC/Opportunity remains observation/product infrastructure, not a strategy by itself.** Empty or
   narrow populations receive no general feature engineering.
...
#### 1.3.1 Profitability task-admission rule *(v0.15)*

Before any new ATP/strategy-program task is started, it must be classified as **STRATEGY-DIRECT** or
**REQUIRED ENABLER** and name its conversion target and stop condition. Work that cannot name a strategy,
economic decision, or minimum necessary evidence dependency is not active ATP work.

This rule changes priority, not scientific standards: governance, admissibility, holdouts, cost models,
and prospective registration remain mandatory when they are necessary to make a profitability claim
credible. The point is to prevent those controls from becoming an independent work program detached
...
| Gate | State (2026-08-27; §0 governs where newer) | Unlocks |
|---|---|---|
...
| **G3 — MDQ-001 verdict (GO/HOLD/STOP)** | **ACTIVE — D0 = 2026-08-19.** First admissible governed frozen partition adjudicated ADMISSIBLE/exit 0. Review window **[2026-08-19, 2026-10-18)**; period holdout **2026-10-06 through 2026-10-17 inclusive**. Capture accrues; final K1–K6 verdict remains future. **v0.16 / A1: expected disposition under the profitability filter is HOLD-with-extension or STOP (ruling O-1); retention decided by §0.3 A2, not by this verdict.** | Permanent SIP adoption path; SCAN-001 SIP migration decision |
...
| **G10 — Phase-B/K2 authorization** | **DEFERRED by default under v0.15 profitability filter** | Open only if a named strategy/overlay requires broad streaming for an economic hypothesis. Do **not** open solely to rescue the MDQ GO floor; otherwise K2 remains NOT EVALUABLE. |
...
### Track 1 — MDQ-001 Phase A *(G1 ✅, G2 ✅, deployed; D0 = 2026-08-19 — heading annotated at v0.16)*
...
### Track 6A — Profit-oriented strategy queue *(v0.15 economic reprioritization; gates unchanged)*

This queue expresses **economic priority**, not authority to bypass pre-registration, admissibility, or
promotion gates. It is intentionally narrower than v0.14: infrastructure/data work is absent unless a
named strategy requires it.

| Priority | Program | Why now | Required proof / stop |
|---|---|---|---|
| **P0** | **LOW-001 governed paper activation / observation** | Closest existing path from completed strategy construction to real economic evidence; currently blocked by deployment-identity closure | S8.6 12/12 + B3a on same runtime, then governed paper results; stop on conformance failure |
| **P1** | **MOM-001 L1 execution enhancement** | Existing alpha should be improved before inventing new alpha | active governed fill/candidate population + lower shortfall/fewer bad executions without signal degradation; otherwise HOLD |
| **P1** | **SF1 NO-START census → one fundamental-change hypothesis** | Cheap way to test whether a distinct alpha path is feasible while GAPPER is data-blocked | population/PIT/missingness/OOS/power gate; zero strategy code until it passes |
| **P1** | **GAPPER data-source qualification** | GAPPER remains economically interesting but 4/250 event-days makes current Stage 0 impossible | one qualified path to ≥250 trustworthy PIT event-days; if none within bounded investigation, STOP this cycle |
| **P2** | **MOM-CAND-001** | Candidate-generation mechanism can be prospectively frozen from Opportunity observations | forward hit-rate / incremental utility independent of plain MOM; untouched prospective test |
...
| **P2 conditional** | **MOM-LIQ-001 / SIP-CONT-001 / SIP-LSR-001** | Use SIP only if current evidence shows economically material information, not merely better data completeness | incremental OOS/walk-forward utility net costs and independence from existing sleeves |
| **Future conditional** | **OPRA-RISK-001 / implied-risk overlays** | Adds a distinct risk dimension only when tied to a named strategy | predeclared interaction + sufficient corpus; no capture-first work |

Portfolio-level priority remains **incremental net utility**, not strategy count. A positive standalone
backtest that adds no net-of-cost portfolio value is rejected.


---

## 6. Recommended execution order — profitability-only current work *(v0.15 text; superseded by §0.4 — item numbers cited elsewhere refer to the v0.14 §6 list)*

Historical item-by-item execution detail is preserved in v0.14. v0.15 carries only work that remains
active under the profitability admission rule.

### 6.1 P0 — unlock and observe a strategy

1. **Protect and close the 2026-08-27 acquisition** under the existing five-gate control. No mutation is
   justified merely to accelerate another track while today's governed evidence is still being earned.
2. After the day closes, obtain the **separate deployment ruling** for Amendment 8 and deploy the exact
   repaired runtime identity package.
3. Verify deployed control version → first governing **READY 6/6** → independently close production
   deployment identity.
4. Open a fresh no-transition window and execute **LOW-001 S8.6 1–12**.
5. Restore rollback baseline only on genuine 12/12 and execute frozen **B3a on the same runtime**.
6. If the LOW-001 activation gates close, begin governed paper observation and measure the strategy's
   economics. The goal is not "strategy running"; it is evidence of net return, drawdown, turnover,
   implementation loss and conformance.

**Why P0:** this chain directly unlocks an already-defined strategy for economic observation. The
deployment-identity work is retained only because it is the minimum necessary enabler.

### 6.2 P0 finish-only — close B-1 without turning MDQ into the product

7. Correct #696's evidence-construction boundary, K1 OR semantics, governed-input authority, and exact
   threshold tests.
8. Land K3 and the K1 frame after immutable review/CI.
9. Run a **bounded provenance-only** search for a pre-corpus authoritative K1 decision provider or
   predeclared defect artifact. Do not inspect K1 outcomes while choosing authority.
10. If no unique pre-existing authority exists, record **K1 = NOT EVALUABLE for this cycle**. Do not
    create a provider/list retroactively and do not spend additional cycles "repairing" the frozen
    criterion.

**Conversion target:** decide whether SIP/MDQ contributes enough economic decision value to justify
continued use. MDQ qualification is not a substitute for strategy validation.

### 6.3 P1 — create the next credible profit paths

11. **Recommend owner sequencing exception: run the SF1 NO-START census now**, not after a potentially
    indefinite GAPPER data wait. It is a 4–6h feasibility test, zero strategy code, and directly answers
    whether one PIT fundamental-change alpha mechanism is worth pre-registering.
12. **GAPPER data-source qualification:** time-box the search for a qualified historical dataset that can
    satisfy the frozen ≥250 trustworthy PIT event-day requirement. If no credible path exists, disposition
    GAPPER as data-blocked / STOP-for-cycle rather than continuing preparation.
13. **MOM-001 L1 execution enhancement:** resume only when a governed population is large enough to measure
    execution economics. If the population remains too small, keep HOLD; do not manufacture observations.
14. **MOM-CAND-001 / RSI-REV-001:** observation/specification may continue only toward a prospectively
    frozen hypothesis with explicit costs, falsification and untouched prospective validation.

### 6.4 Conditional / no active engineering

15. **G10/K2:** no active work solely for MDQ retention. Open only for a named broad-streaming strategy.
16. **DISC-MDQ:** snapshots may accumulate naturally; census may be recomputed cheaply. No broad feature
    library while the population remains empty/narrow.
17. **CEE:** no additional sessions until a viable qualifying fill population exists.
18. **RANGE-SIP-OBS-001:** no work without a new, prospectively named economic mechanism.
19. **Local observation cache / migration ADR:** draft only when a specific L1/L2 strategy migration needs
    it.
20. **OPRA-CAP-001:** no capture-first project. Start only after a named options/risk strategy defines the
    required corpus and expected economic use.

### 6.5 Outside the active strategy queue

Move these to platform/product OPS backlog or let them prove themselves naturally; they are not strategy
priorities:

- wrapper/systemd custody as a standalone PR;
- scheduled-backup-file proof;
- 90-day JSON prune survival;
- branch hygiene / stale-PR bookkeeping;
- plan-location/document cleanup;
- generic feature-library expansion;
- platform refactors with no named strategy consumer.

If any becomes a hard blocker for a P0/P1 strategy, promote **only the minimum blocking slice** back into
the active queue.
...
## 8. Open owner decisions that still matter to profitability *(v0.15)*

The old v0.14 list mixed strategy decisions, infrastructure options, document-location questions and
already-closed governance items. v0.15 keeps only decisions that can change the economic queue.

1. **Amendment 8 deployment authority — OPEN / NEXT after the 08-27 capture closes.** This is a required
   enabler for LOW-001 paper activation, not a platform-improvement project.
2. **K1 provenance disposition — OPEN, bounded.** Determine whether one unique pre-corpus governed
   SCAN/GAPPER decision provider or predeclared defect artifact already exists. If not, K1 is NOT
   EVALUABLE for this cycle. Creating either now is prohibited.
3. **SF1 sequencing exception — RECOMMENDED OPEN DECISION.** Pull the NO-START census ahead of GAPPER
   disposition because GAPPER is data-blocked and the census is a cheap direct test of a new alpha path.
4. **GAPPER data-source path — OPEN, time-boxed.** Either identify a qualified route to ≥250 trustworthy
   PIT event-days or stop the cycle. The threshold is not waivable.
5. **MOM-001 execution-enhancement population — CONDITIONAL.** Resume L1 work only when enough governed
   candidates/fills exist to measure net execution improvement.
6. **DISC-derived strategy graduation — CONDITIONAL.** A candidate family may graduate only through a
   named economic mechanism, ledger citation, multiple-comparison disclosure, prospective
   pre-registration, and untouched prospective validation.
7. **G10/K2 — CLOSED BY DEFAULT FOR STRATEGY PURPOSES.** The 2026-09-21 date does not create work. Open
   only if a named strategy requires broad streaming.
8. **OPRA / live-cache / general feature-library expansion — DEFERRED.** These become owner decisions
   only when a named strategy identifies them as the minimum necessary enabler.

Administrative questions such as plan location, wrapper custody, backup/prune proof and branch hygiene
are **not blockers to the strategy plan**. Track them outside the economic queue.
...
## 10A. Current-version custody rule *(updated at v0.16; 2026-08-27 — v0.15 draft never landed)*

- **v0.15 is the sole active/current implementation plan** once this revision is accepted and lands in Git.
  Its intended canonical path is **`docs/design/ATP/AlgoTraderPlus_v1_4_1_ImplementationPlan_v0_16.md`**.
  ⛔ No second current copy or synchronized mirror.
- v0.14, v0.13, v0.12, v0.11 and all earlier drafts are retained only as historical evidence of what was known at those dates.
- Do not apply operational/state updates to older versions.
- **Operational/state changes update v0.15 IN PLACE**, each tagged with its state-sync date. A new version number is for a later substantive *design* revision, not for a state sync.
- *(v0.15: v0.14 is the immediately prior current-plan record; this successor is a substantive economic-priority revision, not an in-place state sync.)*
...
### v0.15 disposition — profit is the product; everything else is subordinate

v0.14 already said research volume is not the objective, but its active queue still funded work whose
primary result was a better qualification process, broader capability, or cleaner operations. v0.15
makes the economic rule executable.

A task survives in the active queue only when it directly advances a named strategy or is the minimum
necessary enabler to get that strategy into defensible economic observation. This is why the same
revision can **retain** deployment-identity repair (it directly unlocks LOW-001 paper validation) while
**demoting** G10/K2, OPRA capture, broad feature libraries and standalone ops custody (none has a named
near-term strategy conversion target today).

The goal is not to weaken governance. It is to apply governance where it protects an economic claim and
stop treating governance/platform completeness as an independent deliverable.
