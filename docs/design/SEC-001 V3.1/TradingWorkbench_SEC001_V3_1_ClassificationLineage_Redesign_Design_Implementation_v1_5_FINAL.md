...
## Design / Implementation Plan v1.5 — FINAL DESIGN

**Status:** FINAL DESIGN + **WP0A AUTHORIZED**, + **WP0A-Q-COVER AUTHORIZED** (owner, 2026-08-26; see §23–§24) — supersedes v1.4; owner implementation authorization required before Gate 0a / WP0A execution  
**Program:** SEC-001  
...
**v1.5 finalization (2026-08-26):** records the owner's **WP0A-Q-COVER** sub-authority (§24) — a
bounded periodic-cover-page identity acquisition, selected as Option 1 against §23.5 — and freezes the
mandatory pre-acquisition manifest. Also corrects a factual claim this document made: cover-page Inline
XBRL was **phased in 2019–2021, not universally mandatory from 2019** (§24.6). ⛔ **No SEC request has
been issued.**
...
by the registrant itself** — cover-page Inline XBRL (`dei:TradingSymbol`, `dei:Security12bTitle`,
`dei:EntityCentralIndexKey`, exchange), optionally bounded by Form 8-A12B registration and Form 25
delisting records. ⚠ **Corrected in v1.5:** this document originally said "mandatory since 2019". That
is wrong — see §24.6 for the actual phased schedule. The correction does not affect the authorization,
because the binding region is 2022–2026 when the requirement is fully in force. Each such filing is an authoritative statement, at a
...
**DECIDED 2026-08-26 — Option 1.** See §24.

---

## 24. WP0A-Q-COVER — bounded cover-page acquisition sub-authority *(added v1.5)*

### 24.1 Authorization

```text
WP0A-Q-COVER: AUTHORIZED                         owner, 2026-08-26
  scope   : bounded periodic SEC cover-page IDENTITY acquisition for the
            deterministically re-derived WP0A proof population
  purpose : security->CIK qualification ONLY
  status  : sub-authority of WP0A; WP0B / WP1+ remain NOT AUTHORIZED
  terminal: G0A PASS / HOLD / STOP -> custody -> STOP and return to owner
```

Rationale of record: WP0A is at HOLD **only** because the required binding evidence does not exist in
custody. The cover-page evidence is authoritative for this purpose, and acquiring it is far cheaper and
more bounded than letting WP1 begin.

### 24.2 Population — re-derived, not inherited

⛔ Do **not** hard-code the earlier 13, the measured 16, or today's symbol list. The population is
re-derived from sealed inputs under v1.4 rules and frozen in the manifest. The **16-name minimum is a
regression expectation only**; stop-early is decided at run time by the invariance rule.

### 24.3 First tranche — periodic cover pages only

**Authorized forms:** `10-K` · `10-K/A` · `10-Q` · `10-Q/A` · `20-F` · `20-F/A` · `40-F` · `40-F/A` —
the applicable periodic reports whose cover-page information SEC requires to be tagged in Inline XBRL.

⛔ **Not yet acquired:** `8-K` · `8-A12B` · `Form 25` · `6-K` · registration statements · any other
form. These remain possible **second-line** evidence only if the periodic tranche cannot determine G0A,
and **expansion requires a separate owner ruling before any request.**

### 24.4 What a cover filing must prove

A filing is **not** sufficient merely because it contains `EntityCentralIndexKey = CIK` and
`TradingSymbol = XYZ`. Retain the **security-class tuple** per accepted observation:

```text
accepted_at · cik · trading_symbol · security_12b_title · security_exchange_name · form · accession
```

`TradingSymbol` is the symbol of an instrument listed on an exchange; `Security12bTitle` is the title of
a Section 12(b) registered security. The evidence bundle must connect:

```text
permanent security identity -> effective-dated security/class identity
    -> SEC-declared {symbol, security title, exchange} -> registrant CIK
```

⛔ Ticker equality alone remains insufficient **at every hop**. ⭐ For multi-class issuers such as
GOOG/GOOGL the **class-level tuple is decisive** — the same CIK is expected, but each security identity
must match its own declared class and symbol. If existing governed security-identity evidence cannot
establish the first hop without relying solely on ticker equality, that candidate is **DISPUTED**.
⛔ Do not loosen O-9 to close the hop.

### 24.5 Controls that remain unchanged

**Binding predicate** — all three conjuncts of §4.1.2a-1 at exact accepted timestamps. The new
acquisition supplies evidence for the **middle term only**; it does not replace the other two.

**SIC-blind, and this is critical** — the WP0A-Q parser may retain only the approved identity fields.
It must not extract, expose, write, aggregate or inspect SIC from any region of any document. No sector
mapping, prices, returns, P&L or economic information. The return-blind CI guard remains mandatory, and
a **sibling identity-scope guard** must fail closed if the parser emits any field outside the frozen
cover-page schema.

**Defect-E/F transport** — bounded streaming · ignored-Range-200 handling · response/retention ceiling ·
no materialize-then-slice · immutable evidence · fair-access. ⛔ Do **not** fetch an entire Inline-XBRL
10-K wholesale because the needed tags happen to sit on the cover page. The cover facts are usually near
the front of the primary document, but **that is not permission to trust document size**: if the
approved byte ceiling is exhausted before the required fields are found, that observation **fails closed
as evidence-unavailable**.

**CIK-once** — acquire each CIK/form/accession once. ⛔ Do not reacquire the same filing separately for
GOOG and GOOGL or any other shared-CIK pair; one retained cover-page artifact may support multiple
security-binding observations if its class tuples explicitly support them.

**Evidence cutoff** — `WP0A_Q_EVIDENCE_CUTOFF_UTC` is frozen **before the first request**; only filings
**accepted on or before** it are eligible. ⭐ It is deliberately later than the 2026-06-12 evaluation
endpoint: a Q2-2026 report accepted afterwards may legitimately establish that the same
registrant/security relationship continued across a June-2026 week. That later filing is **lineage
evidence, not a return or classification observation.**

**Envelope** — derived from SEC filing **indexes** without inspecting cover-page values. The ~350–480
filing figure is **planning input, not authority**; if the frozen envelope materially exceeds planned
scope, **stop before document acquisition and report**.

### 24.6 📌 Factual correction of record — cover-page XBRL phase-in

This document previously stated that cover-page tagging has been "mandatory since 2019". That is
**incorrect**. SEC phased Inline XBRL cover-page requirements:

| filer class | periods ending on or after |
|---|---|
| large accelerated, U.S. GAAP | **2019-06-15** |
| accelerated, U.S. GAAP | **2020-06-15** |
| all other filers | **2021-06-15** |

SEC guidance expressly covers **10-K, 10-Q, 20-F and 40-F** cover pages, and the taxonomy includes
`EntityCentralIndexKey`, `TradingSymbol`, `Security12bTitle` and exchange information.

⭐ **This does not impair WP0A-Q-COVER**, whose binding region is **2022–2026**, by which time the
requirement is in force for all filer classes. It is recorded so no later reader relies on the wrong
premise — particularly if a future program ever needs cover-page evidence for 2019–2021.

### 24.7 Sealed pre-acquisition manifest

```text
artifact  SEC001_V3_1_WP0AQ_COVER_PREACQUISITION_MANIFEST_V1
path      manifests/wp0aq/WP0AQ_COVER_PREACQUISITION_MANIFEST_V1.json
sha256    3a30ad0296aa945f6b7a68c2bf578e47c69f75267670c2e3d0e72d4473339724
frozen    2026-08-26T16:11:22Z          design commit 741445f
binds     23 candidate identities, deterministic (-cells, permaticker) order,
          worst-case decisive K = 16, evidence cutoff, permitted + deferred forms,
          domains/method/rate/retry/halt, byte ceilings, request caps,
          retained field schema, SIC-blind + return-blind prohibitions,
          CIK-once, stop-early invariance rule, output artifact identities
sec_requests_issued_by_this_manifest: 0
```

⛔ **No population or form-set change is permitted after the first response.**

### 24.8 Terminal outcomes — exactly three

- **A.** Enough bindings proven to make the optimistic Gate-0a STOP **invariant** ⇒ run the governed
  adjudication, custody, return **`G0A_STOP_FROZEN_CLASSES_FORECLOSE_COVERAGE`**. V3.1 then **ends** —
  no WP0B, no WP1.
- **B.** Gate 0a **PASSes** under v1.4 ⇒ custody the PASS and return to the owner checkpoint. ⛔ Do not
  start WP0B automatically.
- **C.** Periodic cover pages insufficient ⇒ return
  **`G0A_HOLD_SECURITY_CIK_BINDING_EVIDENCE_INCOMPLETE`** with exact names, weeks and the reason each
  failed. ⛔ Do **not** automatically expand to 8-K / 8-A12B / Form 25 or any other source — that is a
  separate ruling.

### 24.9 Execution status

**Manifest sealed. No SEC request has been issued.** Execution resumes at the envelope-derivation step:
index-level enumeration of eligible periodic filings for the frozen population under the sealed caps,
checked against planned scope **before** any document acquisition.
