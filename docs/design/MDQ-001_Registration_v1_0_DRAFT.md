...
- **K2 — streaming reliability:** ≥ **99.5%** session uptime over **20 consecutive sessions** at ≥ **250** symbols, zero unrecovered data gaps. *Under the §7 architecture, K2 is measured on the **collector** (Phase B), which is the only process that streams. Phase B starts only when it can run without impairing account-7 execution; if it has not completed 20 sessions by the review date, K2 is scored "not evaluable", not "failed" — the same treatment as K4.*
- **K3 — data completeness:** missing-bar rate reduced ≥ **50%** vs IEX on the qualification universe. *Frozen metric definition (2026-08-15): the comparison grid is the **union** of (symbol, minute) observations across both feeds within the Phase-A bar window (04:00–16:00 ET), excluding minutes where **neither** feed reports a bar; each feed's missing rate = share of grid minutes absent from that feed; K3 is met when `1 − (missing_SIP / missing_IEX) ≥ 0.50`. Raw row-count ratios are **not** the metric — extra SIP bars are not the same thing as reduced missingness.*
...
- **K5 — execution evidence:** spread/mid/shortfall metrics produced for ≥ **90%** of paper fills in the period.
...
5. **Collector ceiling, subordinate to the executor.** The collector carries an explicit resource/rate ceiling (§6.3) and its workload is **subordinate to the account-7 transition executor** — acquisition still spends account 7's data allowance, so contention is bounded at the collector, the single place it can occur.
...
Signing freezes §4 values, §6.3 ceilings, and §7 assignment. After signature, thresholds are immutable for the life of MDQ-001; the only later dispositions are the §5 verdicts.

```
K1–K6 / C1 values:        [ ] accepted as proposed   [ ] adjusted at registration to: ____________
Collector ceiling (§6.3): [ ] accepted as proposed   [ ] adjusted to: ____________
