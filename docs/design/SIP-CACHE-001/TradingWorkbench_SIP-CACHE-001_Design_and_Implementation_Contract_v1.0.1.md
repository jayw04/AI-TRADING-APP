# SIP-CACHE-001 — Operational SIP Data Plane: Design & Implementation Contract
  ...
  Field                               Value
  ----------------------------------- ----------------------------------------------------------------------
  **Document version**                **v1.0.1 — bounded measurement correction (derived from v1.0)**
  ...
  **What v1.0.1 changes**             A **measurement correction only.** The post-freeze entitlement
                                      census (2026-08-31) superseded §2.1's inference that accounts
                                      2–6 were non-entitled, and the owner clarified the commercial
                                      model as **one paid subscription**. v1.0.1 therefore (a) records
                                      the measured census, (b) states the access-vs-subscription
                                      distinction, (c) designates a single authorized SIP producer
                                      identity in §7, (d) adds the no-failover rule to §9/§11/§18, and
                                      (e) updates §0 and §20. **It is NOT an architectural redesign:
                                      the two-plane architecture, SIP_EOD/SIP_LIVE split, freshness
                                      semantics, ADR-0055 boundary, acceptance sequence and every
                                      non-authorization are inherited unchanged.**
                                      **Measured against v1.0, exactly 8 lines are removed** — two
                                      header fields, the superseded §2.1 inference paragraph (4
                                      lines), the superseded §9 `ENTITLEMENT_FAIL` row, and the
                                      superseded §20 open-item row — **and nothing else.** (The
                                      insertion count is deliberately not stated here: writing it into
                                      this row would change it. The exact +/- and the file SHA-256 are
                                      recorded in the custody PR, outside the hashed artifact.)
  ...
  **Canonical repository path**       `docs/design/SIP-CACHE-001/TradingWorkbench_SIP-CACHE-001_Design_and_Implementation_Contract_v1.0.1.md`

  **Supersedes**                      v1.0 for current-state guidance. **v1.0 is NOT mutated:** it
                                      remains as frozen at SHA-256
                                      `5bf7ffb546e5623cfd2655acf41d829d7d3273eb2fa8482ef05b74288f7f4342`.
                                      Its dated assertions are historical evidence and are not
                                      rewritten as though later facts were known at freeze time.
                                      Under ONE CURRENT PLAN semantics, **v1.0 remains CURRENT until
                                      v1.0.1 is actually merged**; CURRENT moves only on merge.
...
| `ATP-SIP-COMMERCIAL-MODEL` | **OWNER-CLARIFIED / ONE PAID SUBSCRIPTION / SHARED WORKBENCH DATA ENABLER** |
| `ATP-SIP-CREDENTIAL-CENSUS` | **THREE BROKER CREDENTIALS CURRENTLY RETURN RECENT-SIP 200 / ACCESS TOPOLOGY OBSERVATION ONLY / DOES NOT ESTABLISH SUBSCRIPTION COUNT** |
| `SIP-CACHE-001 TARGET` | **ONE DESIGNATED SIP PRODUCER IDENTITY → SHARED OPERATIONAL CACHE → GOVERNED CONSUMERS** |
| `SIP-CACHE-001-PRODUCER-001` | **DESIGNATED / WORKBENCH ACCOUNT 7 / BROKER `PA3BGKRLH2AP` / CREDENTIAL FP `b56421a28128` / SHARED MARKET-DATA PRODUCER ONLY / NO STRATEGY EXECUTION AUTHORITY / NO AUTOMATIC FAILOVER** |
...
**Measured entitlement census (supersedes v1.0's inference).** v1.0 recorded accounts 2–6 as
unmeasured and inferred their non-entitlement. That inference was **wrong**, and a bounded read-only
census on 2026-08-31 replaced it with measurement. Credentials were decrypted in memory only via the
canonical `credentials_for_mode()` path; no key or secret was rendered or recorded; each broker
account was identified from Alpaca's own `/v2/account` rather than from a DB row or variable name.

| Workbench acct | Role | Broker (self-reported) | Key fp | SIP | IEX | Classification |
|---|---|---|---|---|---|---|
| 1 | dev | `PA3QRX9KSPXA` | `246b05e74804` | 403 | 200 | NOT_ENTITLED |
| 2 | Range | `PA3NLDNZJQ32` | `4544692ef472` | 403 | 200 | NOT_ENTITLED |
| 3 | ADR-0043 canary | `PA34USW0Q8UO` | `2acf8e9c0c0d` | 403 | 200 | NOT_ENTITLED |
| 4 | forward validation | `PA32AC6G1HB2` | `76662a33f76e` | 403 | 200 | NOT_ENTITLED |
| **5** | sector-rotation (strat 7) | `PA3DBWDGOING` | `5115fc74f097` | **200** | 200 | **ENTITLED** |
| **6** | low-volatility (strat 8) | `PA30T0I3JJV9` | `5da9c9d59a45` | **200** | 200 | **ENTITLED** |
| **7** | combined-book (strat 9) | `PA3BGKRLH2AP` | `b56421a28128` | **200** | 200 | **ENTITLED** |

All four 403s classify as `subscription_not_permitted` with IEX controls at 200 — genuine entitlement
failures, not broken credentials. All seven broker accounts self-report `ACTIVE`.

⚠ **ACCESS IS NOT SUBSCRIPTION COUNT.** Three credentials returning recent-SIP 200 shows only that
three broker credentials currently *have API access* to recent SIP data. Per owner clarification
(2026-08-31) the commercial model is **one paid Algo Trader Plus subscription**, consumed as a shared
Workbench data enabler. **Do not read this table as three paid subscriptions**, and do not infer
authorization, billing correctness, or which credential "should" hold SIP from the shape of the
entitled set.

⭐ **Why v1.0's inference failed.** The 2026-08-15 probe that concluded the subscription had been
*moved, not added* tested only three credentials — unnumbered, `_6`, `_7`. It never tested `_4` or
`_5`. An exclusivity claim was drawn from an incomplete probe set. **An exclusivity claim requires
probing the whole population, not the credentials one happens to suspect.**

**Point-in-time caveat.** This is an entitlement *observation* dated 2026-08-31, not a permanent
property of any account. Re-measure before relying on it.
...
### 7.1 The designated SIP producer identity

SIP is a **shared Workbench data service backed by one paid subscription** (§0), not a per-account
entitlement.
...
The basis is **evidentiary, not convenience**: account 7 is already the identity bound to the entire
MDQ SIP evidence history and to the previously measured ATP/SIP path, so designating it **minimises
provenance discontinuity**.

The designation covers **shared Workbench market-data acquisition only**. It grants Strategy 9 no
additional execution authority, and it does **not** make Strategy 9 the owner of the cache — the cache
is a shared platform service whose consumers are governed independently (§10).

Accounts 5 and 6 remain **observed SIP-capable credentials with no producer role**. This tranche does
**not** revoke, rotate, or otherwise alter them; reconciling why their access exists is separate work
and is not a prerequisite for implementation.

Therefore:

- Exactly **one** credential identity is **designated** as the authorized SIP producer. Acquisition
  for the operational plane uses that identity and no other.
- `entitlement_identity` remains a **per-record provenance field**, not a set of parallel producers.
  It records which identity actually served the observation so that a mis-attributed record is
  detectable — it does not license acquisition from whichever identity happens to work.
- **Strategies never select a SIP credential.** A consumer reads the shared cache. It has no
  credential-selection surface, and must not be able to reach SIP by naming a broker credential of
  its own.
- Strategy 7/8/9 broker credentials remain **order/paper-trading identities only**. That some of them
  currently return recent-SIP 200 (§2.1) is an access-topology observation and confers **no**
  market-data acquisition role.
- Changing the designated producer identity is a **governed change**, recorded with its fingerprint
  and effective timestamp — never an implicit consequence of a credential rotation or an outage.
...
| `ENTITLEMENT_FAIL` | An explicit SIP request **by the designated producer identity** (§7.1) fails because access is unavailable. The shared SIP plane enters this state as a whole; it is never avoided by trying a different credential. |
...
**No failover between SIP-capable credentials.** If the designated paid SIP producer identity fails
entitlement or readiness, the shared SIP plane becomes `ENTITLEMENT_FAIL` and every declaring consumer
fails closed (§10). There is **no automatic failover to accounts 5 or 6** merely because the
2026-08-31 census found their credentials currently return recent-SIP 200. Substituting a working
credential to keep the plane green would silently change the data's provenance, defeat
`entitlement_identity`, and convert an access-topology accident into an undeclared architecture.
Restoring the designated identity is the remedy; swapping identities is not.
...
- **No failover to another SIP-capable credential** when the designated producer identity fails
  (§7.1, §11). `ENTITLEMENT_FAIL` is a fail-closed state, not a prompt to find a credential that works.
- **No consumer-side SIP credential selection.** A strategy reads the shared cache; it never names a
  broker credential to obtain market data.
...
| Accounts 2–6 SIP entitlement | **MEASURED 2026-08-31** — accounts 5, 6, 7 return recent-SIP 200; 1–4 do not (§2.1). Point-in-time observation. |
| Commercial subscription count | **OWNER-CLARIFIED** — one paid subscription; census measures access, not subscriptions (§0, §2.1) |
| Designated SIP producer identity | **RESOLVED 2026-08-31** — `SIP-CACHE-001-PRODUCER-001`, account 7 / `PA3BGKRLH2AP` / fp `b56421a28128` (§7.1) |
| Accounts 5 & 6 SIP access | **OPEN / NOT IN THIS TRANCHE** — observed SIP-capable, no producer role; reconciliation is separate work (§7.1) |
