...
| Status | **PROPOSED GOVERNING DESIGN — APPROVED WITH TWO BLOCKING CORRECTIONS (applied at v0.3); V3-RC run STILL BLOCKED. Disposition 1 was selected and its classification-spine rebuild has been ATTEMPTED AND HALTED — see §5.1c. The §9.4 freeze remains unservable: zero new PIT coverage has been admitted.** |
| Date | 2026-08-25 (v0.4); v0.3 dated 2026-08-23 |
| Supersedes | v0.3 (v0.2 + Review 2 rulings), which superseded v0.2 (v0.1 + Review 1). This v0.4 records the Disposition-1 rebuild attempt and its halt (§5.1c) and corrects two superseded cost estimates in §5.1b. Does **not** rewrite SEC-001 V1/V2 evidence, and amends no §5.1b Q5 coverage figure. |
...
**v0.4 changes (2026-08-25):** adds **§5.1c**, recording the execution status of the Disposition-1 classification-spine rebuild that §5.1b's findings called for. The rebuild has been attempted and is **HALTED at 374 of 1,167 identities**, with **zero admitted terminal credit**, so the §5.1a run blocker and the §9.4 freeze are both **unchanged**. Two §5.1b cost figures are corrected by measurement: the request estimate was low by ~1.9×, and the estimate omitted **retention/storage entirely** — the halt was a disk exhaustion, not a request-budget problem. Nothing in the thesis, constraints, gates, architecture, or non-goals changes; **no coverage figure in §5.1b Q5 is amended**, because no new coverage was produced.
...
**Rebuild cost — ⛔ SUPERSEDED BY MEASUREMENT, see §5.1c. Retained here as the original estimate and
as the record of how it was wrong; do not plan from these figures.** ~~798 distinct names ever entered an annual-grid PIT-200 across 2000–2026 (a weekly rebalance grid will exceed this; treat 798 as a floor). MR-002 averaged ~53 header fetches per CIK over 16 years ⇒ ~106 over 26.5 ⇒ on the order of 85,000 header fetches plus one submissions JSON per CIK — about 2.4 hours of request time at SEC's 10 req/s ceiling, i.e. one unattended crawl.~~

> **Measured correction (v0.4).** Scope was **1,167** identities, not ~798. Requests ran ~**160,000**
> (137/unit measured), not ~85,000 — low by ~1.9×. Most importantly this estimate modelled **request
> time only and omitted retained bytes entirely**; the crawl retains decision bytes by design, needs
> **~295 GB** at the observed amplification, and halted on disk exhaustion at 374/1,167. "One
> unattended crawl" was not achievable as specified.
...
### 5.1c Disposition-1 rebuild — EXECUTION STATUS *(added v0.4, 2026-08-25)*

§5.1b concluded that Disposition 1 was achievable "at overnight-crawl cost" and needed two registered
changes. Disposition 1 was selected and the rebuild was built and run. **It is halted, and it has
produced no usable coverage.** This section records that so the run blocker is not mistaken for
progress.

**Scope as executed.** A frozen identity sequence of **1,167** names (wider than §5.1b's ~798 floor,
as anticipated), crawled in a fixed order from a pinned driver against EDGAR, with per-filing SIC
extracted from the filing's own SGML header by the frozen MR-002 spine. CIK resolution completed
**1,167/1,167**. The 20-F/40-F form-list extension and the 2000-01-01 backward extension were both
carried into the crawl design.

**Outcome: HALTED at 374/1,167** on **2026-08-24 23:15:31 CDT** (`2026-08-25T04:15:31Z`), on
`ENOSPC` — the acquisition volume filled. Terminal ledger is clean (374 records, 374 unique, frozen
order continuous, first identity as frozen), but under the governing epoch ruling the 374 are
**preserved as nonconforming acquisition evidence, not admitted as terminal progress**. Successor
terminal credit is **0**.

**Six acquisition defects were found during the attempt — five are closed; A refuted, B/C/D/E
repaired; F remains OPEN.** They are recorded in dedicated rulings, not here; the sequence matters because it shows the
failure mode:

| defect | substance | status |
|---|---|---|
| A | ranged fallback appeared to recover zero SIC pre-2014 | premise **refuted** — it was E |
| B | `requested_utc` stamped before the throttle slept, so the evidence clock could not substantiate rate compliance | repaired — transmission-time evidence added |
| C | a transport failure inherited the previous attempt's digests and send stamp | repaired |
| D | `HEADER_COMPLETE` reported on EOF without `</SEC-HEADER>`, laundering a source-format anomaly into evidence completeness | repaired — four non-collapsible statuses |
| E | `Accept-Encoding: gzip` + `Range` ⇒ SEC serves a range of the **compressed** representation; a decode failure fell back to raw, so the frozen parser searched gzip noise | **root cause** of the "missing" pre-2014 SIC; repaired |
| F | the Defect-E repair sent `Accept-Encoding: identity`, after which **SEC honoured `Range` zero times** — every ranged request returned `200` with the whole document, and the acquisition loop had no `206` check and no read bound | **open**; caused the halt |

**Defect F is the governing lesson for this program.** The frozen ceiling
(`HEADER_COMPLETION_CAP_BYTES = 1 MiB`, `MAX_REQUESTS = 8`) bounded only *what the client asked for*,
never *what a response could deliver or what was retained*. Measured on the preserved failure state:

```
bytes required to reach the SIC decision   ~39.5 MB      (sum of sec_header_close_offset)
bytes actually retained                    ~94.7 GB      => ~2,400x amplification
largest single retained artifact        422,424,674 B    (header closed at byte 6,195)
```

**Two §5.1b cost estimates are corrected by measurement:**

1. **Requests.** §5.1b estimated ~85,000 header fetches. Measured: **51,336 requests for 374
   units** (~137/unit) ⇒ **~160,000** for 1,167 — low by ~1.9×. Fair-access discipline held
   throughout (≤2.15 req/s, zero 403, zero 429); request rate was never the constraint.
2. **Storage — omitted entirely.** §5.1b modelled request time and concluded "one unattended
   crawl." It modelled **no retention footprint at all**. The actual driver retains decision bytes
   by design (a deliberate evidence invariant), and at the observed amplification the full 1,167
   would require **~295 GB**. The 100 GiB volume could never have completed the run; the halt was
   arithmetically inevitable from the first ranged request.

> **Estimating rule this establishes for the program:** a rebuild estimate for an
> evidence-retaining crawl must model **retained bytes per unit**, not only request count and
> request rate. An acquisition bound that is not enforced on the *response* is not a bound.

**Custody.** The failed epoch is preserved twice: the original volume, still attached to a stopped
host with termination-deletion disabled, and a completed encrypted snapshot. Investigation runs only
against a snapshot-derived read-only copy. Pass-1 storage forensics and Pass-2 byte verification are
both complete and in remote custody on `evidence/sec001-v3-defectF-custody`.

**Pass-2 byte verification — COMPLETE, 100% manifest-to-disk agreement.** All 31,646 complete
manifest records and all 31,030 published artifacts were independently hashed against their recorded
digests on the read-only copy:

```
checked 31,646 · matched 31,646 · mismatched 0 · missing 0 · length mismatches 0
files 31,030 hashed / 31,030 matched / 0 mismatched / 0 missing · 94,290,412,113 bytes
accession-key collisions: 616 total / 616 verified against the shared digest / 0 failed
```

Three conclusions, deliberately kept distinct:

- **Artifact integrity — PASS.** Every retained artifact is the object its manifest record claims.
- **Pass-2 forensic hypothesis ("Defect G", decision-artifact path collision) — CLOSED / REFUTED.**
  ⚠ **This is not a seventh acquisition defect.** A–F above were defects *discovered during the
  attempted crawl*; "G" was a **forensic concern raised afterwards, during Pass-1 analysis**, that
  accession-keyed path collisions might have destroyed evidence. Pass 2 refuted it. The crawl defect
  count remains **six (A–F)**.
  The refutation is arithmetic, not merely an absence of observed mismatches:
  **31,646 manifest records = 31,030 physical artifacts + 616 shared-path collisions**, and all 616
  shared objects independently satisfy *both* of their records' claimed digests.
- **Defect F causation — STRENGTHENED, not closed.** The byte-class census attributes **99.525%**
  of the epoch to published retained artifacts, reconciling exactly with Pass 1's independent total.
  Disk exhaustion came from **unbounded retained response bodies** — not logs, temp files, build
  output, or incidental host usage. **Defect F remains OPEN.**

The damaged manifest tail is bounded and does not contaminate the admissible record stream:
`complete_prefix_sha256 = 772bc5ad…` identifies the 31,646 complete records independently of the
825-byte fragment, and the fragment's surviving `parser_body_sha256` matches its 24,490-byte orphan
artifact exactly — confirming the artifact was written and closed *before* the manifest append that
failed.

⛔ **Pass 2 does not validate SIC correctness, and does not rehabilitate the 374 units.** Byte
integrity is not evidentiary sufficiency; the units were produced under a violated acquisition bound.
Successor terminal credit remains **0/1,167** and §5.1b Q5 is unchanged.

**Consequence for this specification — nothing downstream moves.**

- §5.1a's run blocker is **unchanged**: no reproducible historical PIT sector-classification source
  has yet been proven at the coverage the design requires.
- §5.1b Q5's coverage table is **unamended and remains current**. The held spine still spans
  **2010-01-05 → 2026-07-10**; the first ~10 years of the frozen evaluation period still have no
  coverage, and walk-forward windows 1 and 2 remain essentially uncovered.
- §9.4's freeze therefore still **cannot be served**, and **V3-RC remains blocked**.
- Appendix A's `sector_classification` block stays at `<OWNER_FREEZE>`. Do not populate it from the
  374 partial units.

**What must happen before this section can be closed:** the Defect-F acquisition bound must be
repaired under its own adjudication (including whether the repair reaches the pinned client), a
successor epoch must start at **0/1,167** on clean execution storage, and the crawl must complete
under a canary that specifically proves the bound Defect F defeated. Only then does the coverage
question in §5.1b Q5 get re-measured.
...
Three inputs to the §10 gates freeze with the gates. Two are frozen with values. The third — sector classification (§5.1a) — is **no longer awaiting a disposition ruling: Disposition 1 was selected.** It is instead blocked on *delivery* of that disposition: the classification-spine rebuild is **halted at 374/1,167 with zero admitted coverage (§5.1c)**, so the classification fields cannot yet be frozen and **this freeze cannot be served**. The investigation itself is complete (§5.1b).
...
SEC-001 V3 reaches a **terminal state** by either of two paths. Both are complete outcomes; only the
first involves trading.

**Path 1 — COMPLETE (deployed).** All boxes below.

**Path 2 — COMPLETE (STOP / REDESIGN), per §10.3 and §17.** SEC-001 V3 is equally complete when the
validation run executes as frozen and the §10 primary gates **fail**, provided: V2 is retired and
Account 5 flat with a sealed closeout record; the GO/STOP record discloses the §10.4 ledger count;
no parameter search or in-program tuning was performed; and the result is recorded as a verdict
rather than reopened. A STOP outcome does **not** require paper trading, runtime implementation, or
any §10 gate to pass. Recording a STOP is the deliverable.

> ⚠ **A third state exists today and is neither of the above: BLOCKED.** The validation run has not
> executed, because the classification spine does not exist (§5.1c). Blocked is **not** a STOP —
> a STOP requires the frozen run to have actually produced a failing result. Do not record a STOP
> verdict for an unexecuted run.

Path-1 checklist — SEC-001 V3 is complete (deployed) only when:
...
- [ ] Sector-classification provenance investigation (§5.1a Q1–Q5) is complete (✅ executed 2026-08-23, findings in §5.1b) and its disposition adjudicated (✅ **Disposition 1 SELECTED** — build a genuine effective-dated spine); **the spine is delivered at the coverage §9.4 requires (⛔ NOT MET — rebuild halted at 374/1,167, zero admitted coverage, §5.1c)**; the classification fields in Appendix A are frozen from a proven source — the single-row `tickers.sector` lookup is never labeled PIT.
...
> **⚠ Re-ordered at v0.4.** Items 3–6 below are unchanged in substance but are **all downstream of a
> classification spine that does not yet exist**. §5.1c records that the Disposition-1 rebuild is
> halted with zero admitted coverage, so nothing from item 3 onward can start. The binding path is
> now items 0a–0c.

0. **Unblock the classification spine — the only live critical path:**
   - ~~**0a.** Complete Pass-2 byte verification and adjudicate the Defect-F acquisition bound,
     including whether the repair reaches the pinned client.~~ ✅ **DONE 2026-08-25.** Pass 2:
     100% manifest-to-disk agreement, Defect G closed/refuted. §4.2.1: **NOT SATISFIED** (branch 3 —
     the pinned path has no enforceable consumption ceiling). Ruling §7C sanctions a **V3-local**
     bounded-recording-transport repair, so the remedy does **not** reach the pinned client.
   - **0a′ (NOW).** Implement the §7C repair under its five normative boundaries and prove it with
     the governed canary on accession `0000065984-14-000065` (422,424,674 B; header closes at
     6,195). The proof must demonstrate **bounded transport consumption**, not a bounded artifact
     written after a full network read. ⛔ **Defect F stays OPEN until that canary passes**, and the
     §7C authority covers implementation and canary **only**.
   - **0b.** *(gated on 0a′ PASS)* Rebuild under a successor epoch starting at **0/1,167** on clean
     execution storage.
   - **0c.** Re-measure §5.1b Q5 coverage on the **governed** artifact and bring the §5.1a
     disposition to a final ruling. Only then may Appendix A's `sector_classification` block be
     populated.
...
2. **Freeze V3 v0.4:** approve the proposed profitability gates and construction in this document. *(Freezing the construction does not unblock the run; §5.1a governs that independently.)*
3. **Create `SEC_001_V3` ProgramSpec:** implement the one-candidate research book in Factor Lab. *(blocked on 0c)*
4. **Run GO/STOP validation:** compare only V3 vs V2 reference vs equal-weight control, including 25-bps cost stress and five walk-forward windows. *(blocked on 0c)*
...
**v0.4 additions — the Disposition-1 rebuild attempt (§5.1c).** These are the governing records for
the halted classification crawl; §5.1c summarises them and does not supersede them:

- `SEC001_V3_AcquisitionDefect_F_IgnoredRangeUnboundedRetention_v1_0.md` — the Defect-F ruling,
  **finalized 2026-08-25**. Carries §7B (the §4.2.1 determination, NOT SATISFIED), §7C (the
  `V3_LOCAL_BOUNDED_RECORDING_TRANSPORT_REPAIR` authority, its five normative boundaries and the
  mechanical canary transition condition), and §7.1.a (the Defect-G lineage preserved as a sequence);
- Remote custody on branch `evidence/sec001-v3-defectF-custody`:
  `SEC001_V3_DefectF_SnapshotCustody_Record_v1_0.md` and
  `SEC001_V3_DefectF_StorageForensics_Pass1_v1_0.md` (commit `4e960e2`);
  `SEC001_V3_DefectF_ByteVerification_Pass2_v1_0.md` (`4394eef`);
  `SEC001_V3_DefectF_StreamingPrerequisite_Determination_v1_0.md` (`725e737`);
  `pass2_artifacts/` — four reproducibility artifacts plus a custody manifest (`c95bcbe`).
  The Pass-1 record is sealed **INTERIM / PASS 2 REQUIRED** and was deliberately never rewritten;
  its artifact-integrity statements were superseded by Pass 2, not amended in place;
- `SEC001_V3_CanaryDefect_E_EncodedRangeRepresentation_v1_0.md` and
  `SEC001_V3_CanaryDefect_AcquisitionRemediation_v1_0.md` — Defects A/C/D/E;
- `SEC001_V3_PreCrawl_Manifest_v1_4.json` and `SEC001_V3_ExecutionControl_Addendum_v1_4_1/_1_4_2.md`
  — the authorities under which the halted epoch executed.

⚠ **Custody caveat.** Several SEC-001 V3 governance documents in this set — including this
specification — are **untracked in the working tree** and exist only locally. That is a custody gap
on the documents themselves, independent of the crawl, and is flagged for owner decision.

This specification (v0.3 basis, carried into v0.4) is grounded in:
