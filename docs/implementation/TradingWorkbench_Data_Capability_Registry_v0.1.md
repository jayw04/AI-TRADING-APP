...
| **DCAP-007** | **Alternative-data events (Quiver Quant)** | **event-driven / alternative data** (**paid** — first paid alt-data source) | **Quiver Quant** (`api.quiverquant.com`) — Government Contracts now; Congress / Lobbying / Off-Exchange later (all Hobbyist) | `Settings` env-alias `QUIVER_API_KEY` (`Authorization: Token …`; browser `User-Agent` mandatory for Cloudflare) | **0037 (Accepted)** | **Built (core)** — client + normalizer + ingest merged (#355); migration + USAspending gate pending | **GOVCONTRACT-001** (MVP) |
...
- **DCAP-007 — Quiver Quant.** The platform's **first paid** alt-data source and the **second** event-driven feed (after DCAP-005). It feeds the *existing* PIT Event Store (ADR 0027) with a new `gov_contract_award` event type — **no second store** (ADR 0037). Token auth; a browser `User-Agent` is mandatory (Quiver is behind Cloudflare, which 403s bare clients with `error 1010`). Government Contracts is the GOVCONTRACT-001 MVP dataset; issuer identity resolves through **CAP-024** (Point-in-Time Security Master). ⚠ **Licensing (ADR 0037 §2.4):** Hobbyist **and** Trader both carry "No Commercial Use Rights" — **internal R&D only** until a written Commercial license; no customer-facing cards before then. ⚠ **PIT:** Quiver's row `Date` is a snapshot/record-update date (not a disclosure timestamp), so `available_time` is derived from `action_date` + a conservative disclosure lag, to be **calibrated by the USAspending cross-check** (Phase 1 exit gate).
