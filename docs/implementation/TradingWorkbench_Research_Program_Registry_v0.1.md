...
| **Research Cost Saved** | **4** evidence-based *declines* — RNG-001 (rejected), MOM-002 (enhancement rejected), MF-001 (inconclusive), INSIDER-001 (rejected, beta-not-alpha) — each **prevented the deployment of an unvalidated strategy before capital, live-trading risk, or months of build-out were spent.** Reported as a count of prevented deployments; no dollar/time figure is fabricated. |

### Research Dashboard (one-glance)

The consolidated portfolio view — Status · Verdict · Confidence · Reusable assets in one place (the
detailed Status dashboard follows below). Every value is drawn from this registry; asset counts are shown
only where they are enumerable or owner-provided, else the qualitative **Reuse level** is the reference.

| Program | Status | Verdict | Confidence | Reusable assets |
|---|---|---|---|---|
| **MOM-001** | Production (paper) | ✅ Approved | High | 8 ¹ |
| **RNG-001** | Archived | 🔴 Rejected | High | **7** (enumerated) ² |
| **MOM-002** | Archived | 🔴 Rejected | High | — |
| **MF-001** | Completed | 🟡 Inconclusive | Medium | High reuse (composite engine) |
| **SEC-001** | Archived (constr.) | 🟡 Diversifier (B) | Medium | High reuse (sector-neutral construction) |
| **LOW-001** | Completed | 🟡 Diversifier (B) | Medium | 3 ¹ |
| **TREND-001** | Planning | — Pending | n/a | — |
| **INSIDER-001** | Archived | 🔴 Rejected (C) | High | Very High (Event Store + Event-Study stack) |
| **SCAN-001** ⚙ | Completed (L3) | ✅ Validated (cap.) | High | Very High (every strategy reuses it) |
| **FI-001** 🧩 | Completed | 🟡 Diversifier (B, portfolio) | Medium | Portfolio-Engineering discipline |
| **FI-002** 🧩 | Reserved | — Pending | n/a | — |

*¹ Owner-provided counts (2026-07-01 review). ² RNG-001's seven are enumerated in `RNG-001_Executive_Summary.md`. Where no count is given, the **Reuse level** table and the **Capability Matrix** (CAP-NNN) are the reference — no per-program count is fabricated.*
...
**Evidence Debt** — the gap between a decision's confidence and the evidence behind it: a governance signal
that a conclusion may rest on **insufficient or aging** statistical evidence. The platform's goal is to
*reduce* it. Two forms, one idea:

- **Research Evidence Debt** (this registry) maps directly to the **Confidence** column above: *High*
  confidence (a CI that excludes zero) → **Low** debt; *Medium* confidence (a Diversifier/Inconclusive
  whose CI just spans zero) → **Moderate** debt — the verdict is honestly held, but more evidence would
  sharpen it (e.g. LOW/SEC/TREND/MF/FI-001, and why **FI-002 waits** for live paper data before allocating
  on factor correlation); reserved/unstarted → **n/a**.
- **Live Evidence Debt** (deployed books) — *time since the last meaningful verification* of a live book,
  weighted by how little live evidence has accrued — is owned by the **Continuous Evidence Engine's
  Evidence Clock** (CEE charter §7), not this registry.

High Evidence Debt is not failure; it is a *visible, honest flag* that a decision is provisional and names
...
> **7 chartered** (6 strategy programs + SCAN-001, the platform capability) → strategy verdicts **Approved 1** · **Rejected 1** · **Diversifier 2** · **Inconclusive 1** · **Planned 1**; capability **Validated 1** (SCAN-001)
