# TradingWorkbench — Research Program Registry (v0.19)
...
| Field | Value |
|---|---|
| Version | **v0.19 (2026-07-09)** — reconciled the registry to the Strategy Production Sprint per the `comments.md` review's required follow-up ("the registry is behind the actual research state"): (1) **TREND-001 → Completed · Power-Limited · Inconclusive · Diversifier Candidate** (multi-asset TS trend, 19.3yr; ΔSharpe +0.18 CI [−0.34, +0.71] spans zero, under MDE₉₅ → a *power* failure per pre-reg §9, not an ordinary rejection; maxDD −11.3% vs −30.5%); (2) **TREND-002 added → Completed · Power-Limited · Inconclusive · Diversifier Candidate** (24yr equity+bond core-6; ΔSharpe +0.02 — longer history did not add power, and narrowing the universe *shrank* the edge vs TREND-001's 10-ETF +0.18); (3) **GAPPER-001 added → Running · Active Accrual / Shadow Paper · Backtest Pending** (first Discovery-Lab-born strategy program; v0.2 frozen blind; 20/40 valid dates · 72/100 events; early breakeven read ~11 bps/side is *below* the ≥20 bps/side promotion bar — no verdict until the ≥40-date/≥100-event locked replay ≈ early Aug; **not EAD**); (4) **dashboard recount** — 17 chartered + 2 not-yet-run = **19 total in `programs.py`**, Inconclusive 1 → 3, a new Running bucket, Evidence Neutral 1 → 3; (5) `programs.py` mirrored (TREND-001 verdict updated to the sprint brief, TREND-002 + GAPPER-001 entries added; the 2026-06 per-name 200d-SMA study is recorded as TREND-001's superseded *preliminary* note); (6) sprint side-docs: GAPPER-001 marked **not-EAD** in the Dataset Triage doc; the backfill manifest gained the *original scanner production timestamp* provenance column. _Full changelog (v0.18 → v0.1) preserved in **Appendix A** below._ |
...
| Indicator | Value |
|---|---|
| Programs chartered | **17** (10 strategy [incl. the sprint's **TREND-002** long-history trend + **GAPPER-001** gap-momentum] + 1 external-import [TV-001] + 1 platform capability [SCAN-001] + 1 portfolio-engineering [FI-001] + 1 portfolio-construction [PORT-001, onboarded] + **3 EAD event-driven** [GOVCONTRACT-001 · CONGRESS-001 · LOBBY-001]) + **2 not-yet-run** (FI-002 Correlation Stability *reserved* · FI-003 Tail-Hedge Overlay Validation *planned*) = **19 total in `programs.py`**; EAD reserved: LOBBY-002 · OFX-001. **Reserved EAD items are NOT counted in the 17 chartered programs** — they become chartered only once a pre-registration with a formal program ID exists. |
| Research verdicts | Approved **1** · Diversifier **3** · Inconclusive **3** (MF-001 · **TREND-001 · TREND-002** — the TREND pair *Power-Limited*, secondary **Diversifier Candidate**) · Rejected **7** (RNG-001 · MOM-002 · INSIDER-001 · TV-001 · **GOVCONTRACT-001 · CONGRESS-001 · LOBBY-001**) · **Running 1** (GAPPER-001 — Active Accrual / Shadow Paper · **Backtest Pending**) · Planned/Reserved **2** (FI-002 · FI-003) · Validated capability **2** (SCAN-001 · PORT-001 — construction-validated, not alpha) · *capability-level Rejected* **1** (CAP-020). *(The eight program buckets sum to 19 = the full `programs.py` catalog; CAP-020 is a capability, not a program.)* |
...
| Evidence generated | **Positive 4** (MOM-001 · SCAN-001 · LOW-001 · SEC-001) · **Negative 10** (RNG-001 · MOM-002 · INSIDER-001 · CAP-020 · TV-001 · SCAN v0.1 · SCAN v0.4 · GOVCONTRACT-001 · CONGRESS-001 · LOBBY-001) · **Neutral 3** (MF-001 · TREND-001 · TREND-002 — power-limited, not negative) · **Accruing 1** (GAPPER-001 shadow ledger, no verdict) — *negative evidence is productive: it is the majority of the record and the platform's differentiator* |
...
| **TREND-001** | Trend Following (multi-asset time-series trend, 10-ETF monthly) — **NOT related to TV-001 / Supertrend** | **Completed** | `██████████` 100% | Follow-on (→ TREND-002, run) | 🟡 **Power-Limited · Inconclusive** (secondary: **Diversifier Candidate**) | **Trend philosophy** — the time-series complement, now *evidenced*: Sharpe 0.79 vs 0.61 benchmark, maxDD **−11.3% vs −30.5%** (62.9% relative reduction), but ΔSharpe +0.18 CI [−0.34, +0.71] spans zero *under* MDE₉₅ → a **power failure per pre-reg §9, not a rejection**. Not promoted. ⚠ Still distinct from the (rejected) TV-001-SUPERTREND import candidate — same word "trend", different concepts. |
| **TREND-002** | Long-History Core Trend (equity+bond core-6, 24yr) | **Completed** | `██████████` 100% | Closed (power path exhausted; revisit needs a wider universe or a Calmar-primary design) | 🟡 **Power-Limited · Inconclusive** (secondary: **Diversifier Candidate**) | **The power attack that didn't land — and a real finding**: 24yr/289 rebalances did *not* add power (ΔSharpe +0.02, CI [−0.40, +0.46], MDE₉₅ 0.43), and the trend edge is **thinner on the core-6 than the wider 10-ETF universe** (+0.02 vs +0.18) — narrowing shrank the edge. Drawdown control persists (−17.2% vs −34.7%). Not promoted. |
| **GAPPER-001** ⚙ | Morning gap-momentum continuation — **first Discovery-Lab-born strategy program** (consumes the SCAN-001 gappers feed; **not EAD**) | **Running** | `█████░░░░░` 50% | **Open — Active Accrual / Shadow Paper** | ⏳ **Backtest Pending** (no verdict; no CI computed) | **The Discovery→Strategy pipeline made real**: v0.2 design **frozen blind**, PIT-valid backfill (20/40 dates · 72/100 events, manifest-documented), candidate report → intraday auto-cache → shadow ledger all forward-wired + user-visible ("Morning Opportunities / Backtest Pending"). Early breakeven read ~11 bps/side is *below* the ≥20 bps/side bar → expect Rejected/Inconclusive; the **locked ≥40-date/≥100-event replay (~early Aug) decides**. Do not promote · do not tune · no GAPPER-002. |
...
| TREND-001 | 🟡 Power-Limited · Inconclusive (Diversifier Candidate) | **Medium** | ΔSharpe +0.18, CI [−0.34, +0.71] spans zero, under MDE₉₅; maxDD −11.3% vs −30.5% is the real story (the earlier H1 +0.11 / CI [−0.11, 0.33] per-name figure was a preliminary factor-study note, superseded by this program run) |
| TREND-002 | 🟡 Power-Limited · Inconclusive (Diversifier Candidate) | **Medium** | ΔSharpe +0.02, CI [−0.40, +0.46] spans zero (MDE₉₅ 0.43); ΔCalmar +0.17 CI spans zero; key finding = the edge *thins* as the universe narrows |
| GAPPER-001 | ⏳ Backtest Pending | **n/a** | forward accrual 20/40 dates · 72/100 events; sample gate unmet → **no CI computed, no verdict drawn** (early descriptive read: breakeven ~11 bps/side, below the ≥20 bps/side bar) |
...
| TREND-001 | Portfolio Manager (a return source — now a diversifier candidate blocked on power) | **Medium** |
| TREND-002 | Research Team (the power/universe finding) + Portfolio Manager if revisited | **Medium** (reuses the TREND-001 harness ~wholesale) |
| GAPPER-001 | the **Morning Opportunities** user surface today; Portfolio Manager only if the locked replay approves | **High** (intraday auto-cache + shadow-ledger + PIT-manifest pattern reusable by every future intraday program) |
...
| **TREND-001** | **Power-Limited · Inconclusive (Diversifier Candidate):** 19.3yr multi-asset TS trend — Sharpe 0.79 vs 0.61, maxDD **−11.3% vs −30.5%** (−52.9% SPY), Calmar 0.67 vs 0.25; but ΔSharpe +0.18, CI [−0.34, +0.71] spans zero *and* sits under MDE₉₅ → the pre-registration (§9) requires this be labelled a **power failure**, not counted as an ordinary rejection. Diversifier path: MaxDD-reduction cleared, ΔCalmar CI spans zero. Not promoted. | `evidence/trend_001/TREND001_Evidence_Brief.md` (+ pre-reg v1.0; the 2026-06 per-name study in `evidence/trend_001_trend_following/` is the superseded preliminary note) | **"Directionally favorable ≠ statistically resolved."** The pre-registered power label kept an economically meaningful drawdown story from being either over-claimed (promoted) or over-punished (counted as a rejection). The honest bucket exists and got used. |
| **TREND-002** | **Power-Limited · Inconclusive (Diversifier Candidate):** the deliberate power attack — 24yr/289 rebalances on an equity+bond core-6 — did **not** resolve TREND-001 (ΔSharpe +0.02, CI [−0.40, +0.46], MDE₉₅ 0.43) and surfaced the finding that the trend edge **thins as the universe narrows** (core-6 +0.02 vs 10-ETF +0.18). Drawdown control persists (−17.2% vs −34.7%, 50.4% relative). Not promoted; trend line closed pending a wider universe or a Calmar-primary re-design. | `evidence/trend_002/TREND002_Evidence_Brief.md` (+ pre-reg v1.0) | **Longer history is not more power if the universe shrinks with it** — the edge lives in breadth, not years. A "fix the power" follow-on that honestly reported it didn't, plus a real universe-breadth finding: exactly what a follow-on is for. |
| **GAPPER-001** ⚙ | **Backtest Pending (accruing — no verdict):** v0.2 design frozen **blind**; PIT-valid backfill 20/40 dates · 72/100 events (manifest documents every inclusion + skip); forward pipeline complete (candidate report → intraday auto-cache → shadow ledger, daily jobs live). Early descriptive read: 21/72 triggered, mean daily book gross ~21 bps → breakeven ~11 bps/side, below the ≥20 bps/side promotion bar. **The ≥40-date / ≥100-event locked replay (~early Aug 2026) decides.** | `evidence/gapper_001/` (backfill manifest + shadow-ledger summary; pre-reg v0.2) | **Forward observation without over-claiming**: the product shows "Morning Opportunities / Backtest Pending" to the user while the evidence standard holds — no promotion, no tuning, no GAPPER-002 until the gate. The thin early read is *expected* to end in Rejected/Inconclusive; the discipline, not the signal, is the asset. |
...
- **SCAN-001 → GAPPER-001:** the Discovery Lab's gappers feed → the first Discovery-born *strategy*
  program → blind-frozen v0.2 design + PIT backfill + forward shadow ledger → **Backtest Pending** →
  *generated the intraday auto-cache + shadow-ledger + PIT-manifest pattern (reusable by every future
  intraday program) and the user-visible "Morning Opportunities / Backtest Pending" surface.*
- **TREND-001 → TREND-002:** multi-asset trend power-limited → deliberate long-history power attack →
  **still power-limited**, plus the universe-breadth finding (the edge thins as the universe narrows) →
  *knowledge: "longer history is not more power if the universe shrinks with it."*
...
| Trend following | TREND-001 / TREND-002 | researched (power-limited diversifier candidate; **not shipped**) |
| Intraday gap momentum | GAPPER-001 | accruing (shadow paper; **Backtest Pending**, not shipped) |
...
The pivot point is **TREND-001** *(now run — the TREND pair completed 2026-07-08 through the Strategy
Production Sprint's pre-registered harness, and GAPPER-001 accrues through standing platform jobs rather
than a bespoke study script — the pivot is happening as designed)*. After it, the platform **stops
authoring bespoke research scripts** and
