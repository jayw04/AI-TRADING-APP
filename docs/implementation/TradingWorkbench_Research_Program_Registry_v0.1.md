...
| **CAP-020** | **Market Regime Overlay** (de-risk gross exposure below the market's 200d trend — a return-preserving alternative to continuous vol-targeting; the best drawdown-managed combined book in FI-001) | FI-001 Phase 4 | **Drawdown-effective · Sharpe-neutral · Verdict: Promising · Next: live validation.** Cuts combined-book maxDD −38%→−24% keeping most CAGR, but ΔSharpe CI spans zero and the study is return-level (no turnover cost / single-point calibration). NOT "validated" — the alpha bar is a CI excluding zero; this earns *live* validation, not the word. **🔴 REJECTED (Evidenced) 2026-07-04 — as a Calmar/Sharpe/return improver for the combined book.** Full validation arc: Inconclusive (store 1.5y) → deepened SEP to 2017 (Option A active-names, 7.4y) → **escalated to Option B (full survivorship-free universe, 10,492 tickers)** per the pre-committed rule. **Both universes agree:** ΔCalmar uniformly negative across all 9 grid cells (A −0.22 / B −0.30), ΔSharpe fails guardrail (A −0.15 / B −0.23), ~10pp CAGR give-up, 0/9 robust. Survivorship-free did NOT flip it — the eqw combined book's drawdown (−24.6%) isn't deeper than the biased one, so the "deeper drawdowns rescue it" thesis failed. **Retained finding: the overlay reproducibly works as CRASH INSURANCE** (COVID ΔMaxDD +14.7pp/ΔCalmar +0.25; 2022 +12pp — near-identical in both runs) — spun out to **[[CAP-022]]**. Not deployable as a return enhancer. See `evidence/cap_020/CAP020_Validation_v1.2.md` + `CAP020_DataDeepening_Scope_v0.1.md`. |
| **CAP-022** | **Crash-Insurance / Tail-Hedge Overlay** (the SAME 200d-trend gross de-risk mechanism as CAP-020, but evaluated as a tail hedge, not a return improver — reproducibly cut COVID/2022 drawdowns ~13-15pp) | FI-001 Phase 4 → CAP-020 spin-off | **Planned · Promising.** Acceptance = *does it reduce crash/tail loss enough to justify its cost-of-carry in normal markets?* NOT "does it improve Calmar." Primary metrics: MaxDD reduction in stress regimes (2020/2022-like), CVaR/worst-month improvement, CAGR drag in calm/bull (cost-of-carry), regime-timing false-pos/neg, deployability (cuts live risk w/o excessive return sacrifice). Validated-as-insurance if: materially reduces stress-regime drawdown + improves worst-month/CVaR + bounded calm-regime carry + robust across sweeps + no curve-fit timing. Reuses the deepened survivorship-free store + `scripts/cap020_regime_validation.py` primitives. Charter: `evidence/cap_020/CAP022_CrashInsurance_Charter_v0.1.md`. |
