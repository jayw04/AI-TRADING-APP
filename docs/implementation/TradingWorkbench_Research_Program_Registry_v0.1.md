...
| **CAP-020** | **Market Regime Overlay** (de-risk gross exposure below the market's 200d trend — a return-preserving alternative to continuous vol-targeting; the best drawdown-managed combined book in FI-001) | FI-001 Phase 4 | **Drawdown-effective · Sharpe-neutral · Verdict: Promising · Next: live validation.** Cuts combined-book maxDD −38%→−24% keeping most CAGR, but ΔSharpe CI spans zero and the study is return-level (no turnover cost / single-point calibration). NOT "validated" — the alpha bar is a CI excluding zero; this earns *live* validation, not the word. **Validated 2026-07-04 → Conditionally Promising (Option A), escalating to B.** First run was Inconclusive (store overlap 1.5y); after deepening SEP to 2017 for the active-name universe the window is 7.4y (2019-2026, all 3 environments, 14 flips). Result: ΔCalmar **−0.22** (CI spans 0) uniformly negative across all 9 grid cells, ΔSharpe **−0.15** (fails guardrail), ΔMaxDD +2.2pp (CI spans 0) — **but the overlay clearly works in crashes** (COVID ΔMaxDD +14.75pp/ΔCalmar +0.26; 2022 ΔMaxDD +12pp). Pre-committed escalation rule fires (Conditionally Promising + primary CIs span zero) → **escalate to Option B (survivorship-free universe)**: deeper drawdowns could flip the primary Calmar result. Survivorship-biased active-name universe is not the final evidence package. See `evidence/cap_020/CAP020_Validation_v1.1` + `CAP020_DataDeepening_Scope_v0.1.md`. |
