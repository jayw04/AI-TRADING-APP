...
| **CAP-020** | **Market Regime Overlay** (de-risk gross exposure below the market's 200d trend — a return-preserving alternative to continuous vol-targeting; the best drawdown-managed combined book in FI-001) | FI-001 Phase 4 | **Drawdown-effective · Sharpe-neutral · Verdict: Promising · Next: live validation.** Cuts combined-book maxDD −38%→−24% keeping most CAGR, but ΔSharpe CI spans zero and the study is return-level (no turnover cost / single-point calibration). NOT "validated" — the alpha bar is a CI excluding zero; this earns *live* validation, not the word. **Validation attempted 2026-07-04 → Inconclusive (data-gated):** the box factor store's 4-book overlap is only **1.5y (2024-12→2026-06, bull-only, 1 regime flip)** — a regime overlay can't be validated without a bear regime. Reproducible harness + data-sufficiency gate + owner-approved acceptance framework (Calmar-primary hierarchy + deployment decision matrix) all shipped and **ready to produce a verdict once the store has ≥4y overlapping low-vol/trend/sector history incl. the 2020/2022 bears** (`scripts/cap020_regime_validation.py`; `evidence/cap_020/CAP020_Validation_v1.0.md`). |
