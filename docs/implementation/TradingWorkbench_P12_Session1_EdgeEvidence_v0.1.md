...
| Field | Value |
|---|---|
| Document version | **v0.1 — draft for confirmation** (2026-06-20). Open questions (§"Open questions") to confirm before execution. Drafted against the P12 Direction v0.2 charter + the code survey of the existing backtest/walk-forward/metrics surface. |
| Date | 2026-06-20 |
...
Evidence Package Template, with the evidence-version header, the **Research Registry** row
(Momentum = Validated, with this study as evidence), the **Decision Register** row (Decision =
"baseline established; no enable/disable this session"), and an explicit **Negative/Limitations**
section (survivorship of the live universe, capacity, SPY-window caveat).
