...
## Amendment — 2026-07-30: Layer 2 completes, and a whole-corpus reconstruction is a second construction kind

This ADR listed "**Layer 2 completes**" as a re-evaluation trigger. It has. §4 already said a historical
correction is not a delta; what it did not say is what the correction *is*, or how a runtime built
entirely around `immutable base + ordered deltas` is supposed to load one.

### What Layer 2 found, and why a delta could never have carried it

The countersigned corpus spliced **two adjustment vintages** at 2026-06-15: rows before the seam were
loaded in a one-shot bulk ingest and never re-pulled, so any distribution after that date back-adjusted
only the refreshed side. The effect was not subtle. Measured on the seam date, the cross-sectional mean
one-day adjusted return across 5,761 names was **+38.0398%** — physically impossible — and it compounds
permanently into the cumulative market-proxy index. The same corpus resolved securities by **ticker**,
conflating distinct issuers that reused a symbol (`ECHO` carried EchoStar and Echo Global as one series).

Neither defect is expressible as a delta. A delta appends sessions; these required *re-deriving the
entire history* from a single sealed source vintage under permanent identities. §4 was right, and this
amendment records what the alternative actually looks like.

### The decision

**A whole-corpus reconstruction is a distinct construction kind, loaded natively.**
...
It binds **construction evidence** — crosswalk, both universes, exclusions and price adjudications,
sealed source vintage, normalized corpus evidence, lineage/hole census, the complete adjustment
reconciliation, quarantine, and the ranking/impact analyses — rather than a delta chain. Every digest in
the manifest is **computed from the artifact bytes** at assembly time, and a missing artifact refuses the
build. The manifest must be in its own canonical form: its bytes must re-serialize to themselves, or its
identity cannot be reproduced by anyone else.

**It is NOT converted into a synthetic base-plus-delta manifest.** That was considered and rejected. A
conversion would have to invent a `base_coverage_through`, a base artifact identity and a delta order,
none of which exist — and each invention would be a false statement carried in governed evidence,
indistinguishable downstream from the truth. The runtime accepting the bytes is not the goal; the runtime
accepting them *for the right reason* is.

The loader therefore:

1. recognizes the exact kind and schema version, and **refuses** a version it does not understand rather
   than best-effort parsing a construction whose meaning may have changed;
2. validates the manifest's canonical hash;
3. requires all 18 evidence artifacts, the quarantine histories, both universe identities and the store
   identity;
4. validates the explicit supersession link, including that the prior identity is **unaltered**;
5. returns a normalized construction object that leaves `base_corpus_sha256`,
   `base_coverage_through`, delta order and delta coverage **absent — never defaulted**;
6. leaves base-plus-delta loading entirely unchanged.

Observation and deployment evidence now carry `corpus_construction_kind`,
`construction_schema_version` and `supersedes_corpus_manifest_sha256`.

### Two universes, and two store identities

The reconstruction distinguishes the **mapped-identity universe** (14,145) from the **price-bearing
universe** (14,143). They are never collapsed: the price universe is what governs SEP restriction,
ranking, proxy construction and corpus identity.

§7 retained `store_identity_sha256` as the runtime proof. Layer 2 adds the distinction that a
supersession makes unavoidable:

| identity | what it answers |
|---|---|
| `store_file_sha256` | which physical bytes were shipped |
| `store_identity_sha256` | the registered value-level digest over the consumed window |
| `store_value_identity_sha256` | whole-corpus governed values, **excluding `ingest_runs`** |

The third is an explicitly named **extension**, not the registered convention. It exists because a
rebuild producing identical governed rows with a different physical layout changes the file digest while
preserving the values, and reporting only one cannot tell a re-materialization from a data change.

### Supersession, not mutation

The prior construction keeps its identity `a69ad50ffc3c6925…` **intact and unaltered**. The
reconstruction declares a new identity `1e269fadedff74b0…` and records the old one as superseded with the
reason `HISTORICAL_RECONSTRUCTION_SINGLE_VINTAGE_AND_PERMANENT_LINEAGE`. Editing the old record in place
would destroy the only thing that makes the earlier countersignature checkable.

### What the repair changed, and what it did not

The July 27 portfolio decision is **unchanged** — same five names, same ordering, same equal weights,
same regime state. What changed is a **decision input**: the margin above the regime moving average more
than halved, `+27.2576% → +12.3491%`. The regime state held only because both values sit far from the
±2% band; nearer the band the seam would have changed the regime outright.

That is the honest summary of Layer 2, and it is why "the decision did not change" is the wrong
headline. **A corpus can be wrong in a way that does not change today's answer and would change
tomorrow's.**

### Accepted limitation

The reconstruction does not establish full corporate-action semantics. Eighteen economically terminal
acquired-side events cannot be proven from the vendor schema, which carries no per-share consideration,
exchange ratio or successor conversion term. Each was measured to have zero July 27 decision impact and
is recorded as a **disclosed, session-scoped limitation** — not a waiver, and explicitly not
`PROVEN_NO_PRICE_ADJUSTMENT_APPLICABLE`. The corresponding readiness outcome is a distinct verdict,
`READY_DECISION_VALID_WITH_DISCLOSED_NONDECISION_LIMITATIONS`, which never sets
`adjustment_reflection_proven` and is refused for any session other than the one it names.

### Re-evaluation triggers added

- **A later session replaces 2026-07-27.** The narrow readiness conclusion is scoped to one session by
  construction — the attestation names it and is refused for any other. A new observation must recompute
  its own decision-relevance assessment; nothing about the July 27 result carries forward, and the
  moment anyone wants it to, this amendment is the thing to revisit.
- **A previously unsupported M&A event becomes decision-relevant.** The 18 acquired-side events are
  disclosed *because* each was measured to have zero effect on the decision path. An event of that class
  entering the scoring universe, the proxy contributors, the top five or the regime inputs voids the
  basis for disclosing it, and the corpus is blocked rather than disclosed.
- **SHOP/TLN — or another quarantined identity — receives corrected authoritative history.** The
  quarantine is version-specific, not a permanent universe removal. Corrected vendor history retires it,
  which changes the price universe, the corpus identity and every downstream digest.
- **A second reconstruction is proposed.** This amendment assumes one supersession; a chain of them
  needs a decision about how supersession links compose.
- **A Layer 2 schema version 2 appears.** The loader refuses unknown versions by design; the migration
  path is not described here.
- **The 18 unverifiable acquisitions become verifiable**, through a vendor schema change or a new source.
  The disclosed limitation is sized to one session and should not quietly become permanent.
