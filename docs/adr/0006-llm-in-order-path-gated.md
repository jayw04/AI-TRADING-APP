...
- **Cost scales with market activity**: still true. Mitigated by the per-strategy and per-day budget caps inherited from P3 §4, plus a strict per-user budget cap (default $5/day for opted-in LLM-driven trading; user-configurable upward with an additional confirmation gate). *(Revised 2026-06-05 from the original $10/day to the more conservative $5/day default during P6b §5 implementation, at the owner's instruction; the default is the safe value, and the cap remains user-configurable upward.)*
